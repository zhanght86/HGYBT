...
function LOPrintPreviewQuery() {
   ...
   mSqlStr1 +=" and ((c.appflag = 'B' AND C.UWFLAG = '9') OR";
       mSqlStr1 +=" (C.APPFLAG = '0' AND C.STATE = 'C') OR";
       mSqlStr1 +=" (C.APPFLAG = '0' AND C.STATE = 'B') OR (C.APPFLAG = '1'))";
         if(vManageCodeNo!=""){
          ...
          mSqlStr1+= " AND P.riskcode = '"+vRiskCode+"' ";
           ...
           if(vDay!=""){
          mSqlStr1+= " AND  P.MAKEDATE = DATE '"+vDay+"'";
          }
   ...
   if(mComInfos>2000){
	...
	var winQuery = window.open("LOPreview.jsp?TranCom="+ vTranCom 
		+ "&AreaNo=" + vAreaNo
		+ "&CityNo=" + vCityNo 
		+ "&AgentCom=" + vAgentCom
...
function showRiskCode(){
	fm.RiskCodeNo.value = "";
	fm.RiskCodeName.value = "";
	showCodeList('RiskCode_Print',[fm.RiskCodeNo,fm.RiskCodeName],[0,1],null,null,null,1,null,1);
}

function showRiskCodeKey(){
	fm.RiskCodeNo.value = "";
	fm.RiskCodeName.value = "";
	showCodeListKey('RiskCode_Print',[fm.RiskCodeNo,fm.RiskCodeName],[0,1],null,null,null,1,null,1);
}
function SelectCom(){
