...
function modifydate(strDate)
{
	 ...
	 */
	function date10to8(pDate) {
		if (null==pDate || 10!=pDate.length) {
			return pDate;
		}
	 ...
	 * yyyyMMdd --> yyyy-MM-dd
	 */
	function date8to10(pDate) {
		if (null==pDate || 8!=pDate.length) {
			return pDate;
		}
		
		return pDate.substring(0,4)+'-'+pDate.substring(4,6)+'-'+pDate.substring(6);
	}
