...
////modefied by lilu 20141114
  try {
	...
	//if(sFlag.equals("510015")){
	     String addnum="0";
		 sProviderFormNumber = addnum+request.getParameter("ProviderFormNumber");
...
//****************附加险信息1********************附加险信息1*******************
 System.out.println("****************附加险信息1*******************附加险信息1*******************");
	...
	//贷款合同号
	String sContractNo = request.getParameter("LoanNo");
	//贷款账号
	String sLoanAccountNo = request.getParameter("AccNo");
	//借款金额
	String sLoanAmount = request.getParameter("LoanPrem");
	//贷款机构
	String sLoanBank = request.getParameter("LoanBank");
	//贷款起始日期
	String sLoanStartDate = request.getParameter("LoanDate");
	...
	//保险合同生效日期
	String sContractEffDate = request.getParameter("InsuDate");
	sContractEffDate=DateUtil.date10to8(sContractEffDate);
	//保险合同到期日期
	String sContractEndDate = request.getParameter("InsuEndDate");
	sContractEndDate=DateUtil.date10to8(sContractEndDate);
	//贷款种类
	String sLoanProductCode = request.getParameter("LoanType");
	...
	}else if(sProductCode.equals("0002")){
		if(sModalPremAmt == null || "".equals(sModalPremAmt)){
			sModalPremAmt="0";
		}
...
ElementLis PbBenfIdEndDate = new ElementLis("PbBenfIdEndDate","20201231",Benf_Detail);
...
if(!(sBnfFullName3==null||sBnfFullName3.equals(""))){
	...
	//险种代码
	//处理险种代码
	if("0001".equals(sProductCode))
	{
		sProductCode = "231204";
	}
	else if("0002".equals(sProductCode))
	{
		sProductCode = "211902";
	}
	else if("0003".equals(sProductCode))
	{
		sProductCode = "221201";
	}
	else if("0004".equals(sProductCode))
	{
		sProductCode = "231302";
	}
	else if("0005".equals(sProductCode))
	{
		sProductCode = "221203";
	}
	else if("0006".equals(sProductCode))
	{
		sProductCode = "225501";
	}
...
//如果有附加险2  
	...
	if(sProductCode.equals("0002")){
		//贷款信息
		ElementLis ContractNo = new ElementLis("LoanNo",sContractNo,Transaction_Body);
					ElementLis LoanAccountNo = new ElementLis("AccNo",sLoanAccountNo,Transaction_Body);
					ElementLis LoanAmount = new ElementLis("LoanPrem",sLoanAmount,Transaction_Body);
					ElementLis LoanBank = new ElementLis("LoanBank",sLoanBank,Transaction_Body);
					ElementLis LoanStartDate = new ElementLis("LoanDate",sLoanStartDate,Transaction_Body);
					ElementLis LoanEndDate = new ElementLis("LoanEndDate",sLoanEndDate,Transaction_Body);
					ElementLis ContractEffDate = new ElementLis("InsuDate",sContractEffDate,Transaction_Body);
					ElementLis ContractEndDate = new ElementLis("InsuEndDate",sContractEndDate,Transaction_Body);
					
					ElementLis LoanProductCode = new ElementLis("LoanType",sLoanProductCode,Transaction_Body);
	}
...
//	ElementLis BkRckrNo = new ElementLis("BkRckrNo",sTeller,Transaction_Body);
	...
	ElementLis PbRemark6 = new ElementLis("PbRemark6","",Transaction_Body);
        ...
        System.out.println("-----------开始取数（返回报文页面）----------");
			JdomUtil.print(mOutXmlDoc);
		ResultCode = mOutXmlDoc.getRootElement().getChild("Tran_Response").getChildTextTrim("BkOthRetCode");
		ResultInfoDesc = mOutXmlDoc.getRootElement().getChild("Tran_Response").getChildTextTrim("BkOthRetMsg");
