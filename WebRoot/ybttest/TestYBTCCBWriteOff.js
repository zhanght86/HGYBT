function WTranFlag(){
	...
	if(sTranFlagCode == 'OPR911'){
		//当日撤单
		ContConcel();
	}
	else if(sTranFlagCode == 'SPE802'){
		//当日冲正
		ContRollBack();
	}
	else if(sTranFlagCode == 'SPE002'){
		//保单重打
		ContRePrint();
	}
	else if(sTranFlagCode == 'OPR999'){
		//绿灯交易
		GreenTest();
	}else if(sTranFlagCode == 'SPE801'){
		//退保交易传递
		ContCancelBlc();
	}else if(sTranFlagCode == 'VCH102'){
		//重空核对
		ContCheck();
...
function  verifyInputForm(){
	...
	//alert(vtranFlagCode);
	if (vPolNumber.trim() == "" && (vtranFlagCode.trim() == "OPR911")){    
		alert("当日撤单交易时保单号不能为空");
		return false;   
	} 
	if (vOldTransRefGUID.trim() == "" && (vtranFlagCode.trim() == "OPR911")){    
		alert("当日撤单交易时原交易流水号不能为空");
		return false;   
	} 
		if (vPolNumber.trim() == "" && (vtranFlagCode.trim() == "SPE002")){    
		alert("保单重打交易时保单号不能为空");
		return false;   
	}

		if (vProviderFormNumber.trim() == "" && (vtranFlagCode.trim() == "SPE002")){    
