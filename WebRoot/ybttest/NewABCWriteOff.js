function WTranFlag(){
	...
	if(sTranFlagCode == '1010'){
		//当日撤单
		ContConcel();
	}
	else if(sTranFlagCode == '1009'){
		//当日冲正
		ContRollBack();
	}
	else if(sTranFlagCode == '1018'){
		//保单重打
		ContRePrint();
	}
	else if(sTranFlagCode == '1005'){
		//试算结果查询
		ApplyResult();
	}
	else if(sTranFlagCode == '1000'){
		//绿灯交易
		GreenTest();
	}
}
...
function ApplyResult(){
	...
	//试算结果查询
	document.getElementById("divApplyNo").style.display="";
	document.getElementById("divApplyNoimg").src="../common/images/butCollapse.gif";
...
//	}
	...
	//alert(vtranFlagCode);
	if (vPolNumber.trim() == "" && (vtranFlagCode.trim() == "1010")){    
		alert("当日撤单交易时保单号不能为空");
		return false;   
	} 
	if (vOldTransRefGUID.trim() == "" && (vtranFlagCode.trim() == "1010")){    
		alert("当日撤单交易时原交易流水号不能为空");
		return false;   
	} 
//		if (vPolNumber.trim() == "" && (vtranFlagCode.trim() == "1018")){    
//		alert("保单重打交易时保单号不能为空");
//		return false;   
//	}

		if (vProviderFormNumber.trim() == "" && (vtranFlagCode.trim() == "1018")){    
