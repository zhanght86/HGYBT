...
<OLifE>
	<Holding id="cont">   
		...
		<Policy>
			...
			<!--  首期保费-->
			<PaymentAmt><xsl:value-of select="$MainRisk/Prem"/></PaymentAmt> <!-- 本期保险费合计 首期保费金额(字节,带小数点) --> 
 			<!--  银行帐户20字节 -->
 			<AccountNumber><xsl:value-of select="$MainRisk/AccountList/Account/AccNo"/></AccountNumber>  
 			<!--  帐户姓名10字节 -->     
			...
			<OLifEExtension>
				...
				<CovPeriod></CovPeriod>
				<!-- 总保费 -->
				<GrossPremAmt><xsl:value-of select="Prem"/></GrossPremAmt>
			</OLifEExtension>
		</Policy>
		<Investment id="Invest_1">
					<SubAccountCount>1</SubAccountCount>
					<!--投资帐户个数-->
					<SubAccount id="SubAcct_1">
						<ProductCode></ProductCode>
						<!--投资帐户代码-->
						<ProductFullName><xsl:value-of select="$MainRisk/AccountList/Account/AccName"/></ProductFullName>
						<!-- 投资账户名称 -->
						<AllocPercent>100</AllocPercent>
						<!-- 投资账户比率 -->
						<CurrNumberUnits></CurrNumberUnits>	
						<!--投资单位数-->
						<UnitValue></UnitValue>
						<!--投资单位卖出价-->
						<UnitValueBuyRate></UnitValueBuyRate>
						<!--投资单位买入价-->
						<PayoutUnitValue></PayoutUnitValue>
						<!--投资单位价值-->
						<TotValue></TotValue>
						<!--投资帐户总市值-->
					</SubAccount>
...
<xsl:attribute name="id"><xsl:value-of select="concat('risk_', string(position()))" /></xsl:attribute>
	...
	<Duration><xsl:value-of select="Years"/></Duration>
	<OLifEExtension>
		<xsl:choose>
...
<xsl:choose>
	...
	<xsl:when test=".=321020">002</xsl:when>	<!-- 中融融盛连年终身寿险（万能型）A款 -->
	<xsl:when test=".=313020">003</xsl:when>	<!-- 中融融鑫添福两全保险（分红型）B款 -->
	<xsl:when test=".=313030">004</xsl:when>	<!-- 中融融华富贵两全保险（分红型）A款 -->
