...
<!-- 保单传送方式 -->
<GetPolMode><xsl:value-of select="$HoldingNode/Policy/OLifEExtension/PolicyDeliveryMethod" /></GetPolMode>
...
<Appnt>
	...
	<JobCode>3010101</JobCode>  <!-- 核心默认值 代码为3010101 一般内勤-->
	<Company><xsl:value-of select="$PartyNode/Person/Company" /></Company>
	<Nationality><xsl:apply-templates select="$PartyNode/Person/NationalityNo" /></Nationality>
...
<Insured>
	...
	<Nationality><xsl:apply-templates select="$PartyNode2/Person/NationalityNo" /></Nationality>
...
<Risk> 
	...
	<xsl:choose>		
		<xsl:when test="OLifEExtension/DurationMode=5">	<!-- 保终身 -->
			...
			<!-- 保险年期年龄 -->
			<InsuYear><xsl:value-of select="OLifEExtension/Duration" /></InsuYear>         <!-- modified in 20121029 -->
		</xsl:when> 
...
<xsl:choose> 
	...
	<!-- 国籍 -->
<xsl:template name="tran_nationality" match="NationalityNo">
<xsl:choose> 
...
<xsl:choose>
	...
	<xsl:when test=".=009">221301</xsl:when> <!-- 中韩悦未来年金险-->
...
<xsl:template name="tran_LoanType" match="LoanProductCode">
	<xsl:choose>
		<xsl:when test=".=1">06</xsl:when>	<!-- 短期个人自用车贷款 -->
		<xsl:when test=".=3">07</xsl:when>	<!-- 短期个人综合消费贷款 -->
		<xsl:when test=".=4">08</xsl:when>	<!-- 短期个人信用贷款 -->
		<xsl:when test=".=5">09</xsl:when>	<!-- 短期个人质押贷款 -->
		<xsl:when test=".=6">10</xsl:when>	<!-- 短期个人房屋抵押贷款 -->
		<xsl:when test=".=7">11</xsl:when>	<!-- 短期个人经营贷款 -->
		<xsl:when test=".=8">12</xsl:when>	<!-- 中长期个人经营贷款 -->
		<xsl:when test=".=9">13</xsl:when>	<!-- 短期下岗失业人员小额担保贷款 -->
		<xsl:when test=".=10">14</xsl:when>	<!-- 中央贴息国家助学贷款 -->
		<xsl:when test=".=11">15</xsl:when>	<!-- 地方贴息国家助学贷款 -->
		<xsl:when test=".=12">16</xsl:when>	<!-- 一般商业性助学助学贷款 -->
		<xsl:when test=".=13">17</xsl:when>	<!-- 中长期下岗失业人员小额担保贷款 -->
		<xsl:when test=".=14">18</xsl:when>	<!-- 短期个人商用车贷款 -->
		<xsl:when test=".=15">19</xsl:when>	<!-- 个人小额贷款 -->
		<xsl:when test=".=17">20</xsl:when>	<!-- 个人一手住房购置贷款（含自建房） -->
		<xsl:when test=".=18">21</xsl:when>	<!-- 个人商用房贷款 -->
		<xsl:when test=".=19">22</xsl:when>	<!-- 个人住房装修贷款 -->
		<xsl:when test=".=20">23</xsl:when>	<!-- 个人家居消费贷款 -->
		<xsl:when test=".=21">24</xsl:when>	<!-- 个人二手住房购置贷款 -->
		<xsl:when test=".=23">25</xsl:when>	<!-- 其他个人住房贷款 -->
		<xsl:when test=".=26">26</xsl:when>	<!-- 中长期个人自用车贷款 -->
		<xsl:when test=".=27">27</xsl:when>	<!-- 中长期个人大额耐用品贷款 -->
