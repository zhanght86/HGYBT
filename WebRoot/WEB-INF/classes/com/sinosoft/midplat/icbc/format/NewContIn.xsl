...
<JobNotice><xsl:value-of select="$HoldingNode/Policy/OLifEExtension/OccupationIndicator" /></JobNotice> 
 ...
 <!-- 销售人员工号 -->
 <SaleStaff><xsl:value-of select="/TXLife/TXLifeRequest/OLifEExtension/Teller" /></SaleStaff>
 <!-- 销售人员姓名 -->
...
<Insured>
	...
	<JobCode><xsl:value-of select="$PartyNode2/Person/OccupationType" /></JobCode>  <!-- 核心默认值 代码为3010101 一般内勤    20150105 新增产品需要职业代码，工行采用核心编码，不需转换 -->
...
<xsl:variable name="HoldingNode3" select="../Holding[@id=$HoldingID3]/Policy" />
	...
	<xsl:choose> 
		 <!-- 中韩安赢借款人意外伤害保险 -->
		<xsl:when test = "/TXLife/TXLifeRequest/OLifE/Holding/Policy/Life/Coverage[IndicatorCode='1']/ProductCode='004' and string-length($PartyNode/FullName)!=0">
		<Bnf>
		 ...
		 <!-- 中韩安赢借款人意外伤害保险A款 -->
		<xsl:when test = "/TXLife/TXLifeRequest/OLifE/Holding/Policy/Life/Coverage[IndicatorCode='1']/ProductCode='008' and string-length($PartyNode/FullName)!=0">
		<Bnf>
			<Type>1</Type>	<!-- 默认为“1-死亡受益人” -->
			<Grade>2</Grade>
			<Name><xsl:value-of select="$PartyNode/FullName" /></Name>
		...
		<xsl:when test="/TXLife/TXLifeRequest/OLifE/Holding/Policy/OLifEExtension/BeneficiaryIndicator='N' and /TXLife/TXLifeRequest/OLifE/Holding/Policy/Life/Coverage[IndicatorCode='1']/ProductCode !='004' and /TXLife/TXLifeRequest/OLifE/Holding/Policy/Life/Coverage[IndicatorCode='1']/ProductCode !='008' ">
		<Bnf>
			<Type>1</Type>	<!-- 默认为“1-死亡受益人” -->
...
<Risk> 
			...
			<!-- 主险险种代码 -->
			<MainRiskCode><xsl:apply-templates select="/TXLife/TXLifeRequest/OLifE/Holding/Policy/Life/Coverage[IndicatorCode='1']/ProductCode"/></MainRiskCode>
			<!-- 保额  借贷险的时候取贷款信息部分保险金额-->
			<xsl:choose>
				<xsl:when  test = "ProductCode='004'">
				<Amnt><xsl:value-of select="/TXLife/TXLifeRequest/OLifE/Holding/Policy/OLifEExtension/FaceAmount"/></Amnt>
				</xsl:when>
				<xsl:when  test = "ProductCode='008'">
				<Amnt><xsl:value-of select="/TXLife/TXLifeRequest/OLifE/Holding/Policy/OLifEExtension/FaceAmount"/></Amnt>
				</xsl:when>
				<xsl:otherwise>
...
<xsl:template match="OLifEExtension">
		<Loan>
			...
			<!-- 贷款种类  工行有的-->
			<LoanType><xsl:apply-templates select="LoanProductCode" /></LoanType>
			<!-- 贷款账号 -->
...
<xsl:choose>
	...
	<xsl:when test=".=013">221301</xsl:when> <!-- 中韩悦未来-->
...
<xsl:choose>
	...
	<xsl:when test=".=5">Y</xsl:when>	<!-- 趸交交费年期传Y -->
...
<!-- 借贷险贷款种类  lilu 20141127 注释前面有字的为原来的，右侧多出来的为新增的，新增的前面是不要的 -->
<xsl:template name="tran_LoanType" match="LoanProductCode">
	<xsl:choose>
		<xsl:when test=".=030100000005">06</xsl:when>	<!-- 1 短期个人自用车贷款 -->
		<xsl:when test=".=030100000008">07</xsl:when>	<!-- 3 短期个人综合消费贷款 -->
		<xsl:when test=".=030100000011">08</xsl:when>	<!-- 4 短期个人信用贷款 -->
		<xsl:when test=".=030100000012">09</xsl:when>	<!-- 5 短期个人质押贷款 -->
		<xsl:when test=".=030100000007">10</xsl:when>	<!-- 6 短期个人房屋抵押贷款 -->
		<xsl:when test=".=030100000001">11</xsl:when>	<!-- 7 短期个人经营贷款 -->
		<xsl:when test=".=030100000102">12</xsl:when>	<!-- 8 中长期个人经营贷款 | 个人文化消费贷款-->
		<xsl:when test=".=030100000002">13</xsl:when>	<!-- 9 短期下岗失业人员小额担保贷款 -->
		<xsl:when test=".=030100000019">14</xsl:when>	<!-- 10 中央贴息国家助学贷款 -->
		<xsl:when test=".=030100000017">15</xsl:when>	<!-- 11 地方贴息国家助学贷款 -->
		<xsl:when test=".=030100000018">16</xsl:when>	<!-- 12 一般商业性助学助学贷款 -->
		<xsl:when test=".=030100000041">17</xsl:when>	<!-- 13 中长期下岗失业人员小额担保贷款 | 个人助业贷款-->
		<xsl:when test=".=030100000006">18</xsl:when>	<!-- 14 短期个人商用车贷款 -->
		<xsl:when test=".=030100000025">19</xsl:when>	<!-- 15 个人小额贷款 | 个人委托个人贷款-->
		<xsl:when test=".=140010000296">20</xsl:when>	<!-- 17 个人一手住房购置贷款（含自建房） -->
		<xsl:when test=".=030100000004">21</xsl:when>	<!-- 18 个人商用房贷款 -->
		<xsl:when test=".=030100000022">22</xsl:when>	<!-- 19 个人住房装修贷款 |公积金委托贷款-->
		<xsl:when test=".=030100000013">23</xsl:when>	<!-- 20 个人家居消费贷款 -->
		<xsl:when test=".=030100000020">24</xsl:when>	<!-- 21 个人二手住房购置贷款 -->
		<xsl:when test=".=030100000101">25</xsl:when>	<!-- 23 其他个人住房贷款 | 个人游艇贷款-->
		<xsl:when test=".=030100000014">26</xsl:when>	<!-- 26 中长期个人自用车贷款 | 个人留学贷款-->
		<xsl:when test=".=030100000021">27</xsl:when>	<!-- 27 中长期个人大额耐用品贷款 | 建房贷款 -->
		<xsl:when test=".=030100000103">12</xsl:when>	<!-- 企业委托个人贷款 -->
		<xsl:when test=".=030410000021">12</xsl:when>	<!-- 委托转让贷款 -->
		<xsl:when test=".=030100000121">12</xsl:when>	<!-- 逸贷 -->
		<xsl:otherwise></xsl:otherwise>  
	</xsl:choose>
</xsl:template>
