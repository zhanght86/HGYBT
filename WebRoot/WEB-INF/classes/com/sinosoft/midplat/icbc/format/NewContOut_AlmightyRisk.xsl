				...
				<PageNum>1</PageNum>
					...
					<TextRowContent><xsl:text></xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(RiskName, 36)"/>
																			...
																			<xsl:apply-templates select="PayIntv"/>
																								 <xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_($Amnt,16,$Falseflag)"/><xsl:text>元</xsl:text>
...
<xsl:choose>
	<xsl:when test=".=2241201">012</xsl:when>	<!-- 中韩创赢财富两全保险（万能型）A款 -->
	<xsl:otherwise>--</xsl:otherwise>
</xsl:choose>
</xsl:template>
