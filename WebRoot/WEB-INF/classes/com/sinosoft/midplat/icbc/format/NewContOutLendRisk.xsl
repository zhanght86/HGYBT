	...
	<Print>
		...
		<SubVoucher>
			...
			<Text>
				...
				<PageNum>1</PageNum>
				 ...
				 <TextRowContent><xsl:text></xsl:text>伤残第二顺序受益人为被保险人本人。</TextRowContent>
				 </xsl:when>
				 <xsl:otherwise>
				 <TextRowContent>
				 ...
				 <TextRowContent><xsl:text></xsl:text>全残第二顺序受益人为被保险人本人。</TextRowContent>
				 </xsl:otherwise>
				 </xsl:choose>
				 <xsl:variable name="num" select="count(Bnf)" />
				 <xsl:choose>
				 <xsl:when test="$num = 3">
				 <TextRowContent><xsl:text></xsl:text>身故第二顺序受益人：法定</TextRowContent>
				 <TextRowContent/>
				 <TextRowContent/>
				 </xsl:when>
				 <xsl:otherwise>
				...
				<xsl:choose>
				 ...
				 <TextRowContent>
					...
					<TextRowContent />
					<TextRowContent><xsl:text></xsl:text>------------------------------------------------------------------------------------------------</TextRowContent>
					<TextRowContent><xsl:text></xsl:text>险种名称                          保险期间    交费期间    交费方式  （基本）保险金额/份数  (首期)保险费</TextRowContent>
					...
					<TextRowContent />
					<TextRowContent />
					<TextRowContent><xsl:text>保险期间起止时间：</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.DateUtilZR.date8to11(Risk/CValiDate)"/><xsl:text>零时起至</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.DateUtilZR.date8to11(Risk/InsuEndDate)"/><xsl:text>二十四时止</xsl:text></TextRowContent>  
		...
		<Text> 
					      ...
					      <TextRowContent/>
						   ...
						   <TextRowContent><xsl:text></xsl:text><xsl:text>    本投保人已收到贵公司的保险合同（保险合同号:</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(ContNo, 17)"/><xsl:text>），本保险合同包括保险单、保</xsl:text></TextRowContent>
						   <TextRowContent><xsl:text></xsl:text><xsl:text>险条款等相关资料，经审核确认保险合同内容正确无误。本人已阅读过产品条款、投保提示书，确认已了</xsl:text></TextRowContent>
						   <TextRowContent><xsl:text></xsl:text><xsl:text>解并认可保险合同的全部内容，知晓本人的权利和义务。</xsl:text></TextRowContent>
