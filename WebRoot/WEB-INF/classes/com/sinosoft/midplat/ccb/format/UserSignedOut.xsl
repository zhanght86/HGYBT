	...
	<xsl:template match="/">
		<Transaction>
			...
			<Transaction_Body>
				...
				<BkDetail1>
				<xsl:text>　　　　</xsl:text><xsl:text>银行账号：</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(TranData/Body/BankAccNo, 41)"/>
				<xsl:text>　　　　        </xsl:text><xsl:text>生效日期：</xsl:text><xsl:value-of select="TranData/Body/CValidate"/>
				</BkDetail1>
							 <BkDetail1></BkDetail1>
								 <BkDetail1></BkDetail1>
