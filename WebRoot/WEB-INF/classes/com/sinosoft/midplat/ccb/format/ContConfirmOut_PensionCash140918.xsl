			...
			<Transaction_Body>
				...
				<xsl:if test="$MainRisk/CashValues/CashValue != ''">	 
					...
					<Detail>
					 ...
					 <BkDetail1><xsl:text></xsl:text>                            现金价值及减额交清表                     </BkDetail1>
					 <BkDetail1/>
					 <BkDetail1><xsl:text></xsl:text>保险合同号：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(TranData/Body/ContNoContNo, 50)"/><xsl:text></xsl:text>基本保险金额： <xsl:value-of select="$Amnt"/></BkDetail1>
					 <BkDetail1><xsl:text></xsl:text>险种名称：<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_($MainRisk/RiskName, 52)"/>货币单位：人民币/元</BkDetail1>
					 <BkDetail1><xsl:text></xsl:text>------------------------------------------------------------------------------------------------</BkDetail1>
					 <BkDetail1 >保单年 <xsl:text>         </xsl:text>减额交清后|保单年<xsl:text>          </xsl:text>减额交清后|保单年<xsl:text>         </xsl:text>减额交清后</BkDetail1>
					 <BkDetail1 >度末 <xsl:text> </xsl:text>现金价值<xsl:text>  </xsl:text>的基本保额|度末 <xsl:text> </xsl:text>现金价值<xsl:text>  </xsl:text>的基本保额|度末 <xsl:text> </xsl:text>现金价值<xsl:text>  </xsl:text>的基本保额</BkDetail1>
					 <BkDetail1><xsl:text></xsl:text>------------------------------------------------------------------------------------------------</BkDetail1>
