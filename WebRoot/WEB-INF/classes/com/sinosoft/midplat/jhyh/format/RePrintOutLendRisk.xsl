...
<xsl:variable name="Falseflag" select="java:java.lang.Boolean.parseBoolean('false')" />
		...
		<xsl:text>※※  </xsl:text>  保险合同成立日：<xsl:value-of select="java:com.sinosoft.midplat.common.DateUtil.getCur10Date()"/><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(' ', 38)"/>保险合同生效日期：<xsl:value-of select="java:com.sinosoft.midplat.common.DateUtil.date8to10(/TranData/Body/Risk/CValiDate)" />
	    <xsl:text>※  </xsl:text><xsl:text>  ------------------------------------------------------------------------------------------------</xsl:text>
		...
		<xsl:text>※  </xsl:text><xsl:text>  被保人姓名：</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(Insured/Name, 18)"/>
			 ...
			 <xsl:choose>
				 <xsl:when test ="$MainRisk/RiskCode=211902" >
				    <xsl:text>※  </xsl:text><xsl:text>  身故/伤残第一顺序受益人：</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(Bnf[Grade='1']/Name, 40)"/><xsl:text>受益比例：</xsl:text><xsl:value-of select="Bnf[Grade='1']/Lot"/><xsl:text>%</xsl:text>
				    <xsl:text>※  </xsl:text>  伤残第二顺序受益人为被保险人本人。<xsl:text></xsl:text>
				 </xsl:when>
				 <xsl:otherwise>
				    <xsl:text>※  </xsl:text><xsl:text>  身故/全残第一顺序受益人：</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(Bnf[Grade='1']/Name, 40)"/><xsl:text>受益比例：</xsl:text><xsl:value-of select="Bnf[Grade='1']/Lot"/><xsl:text>%</xsl:text>
				   <xsl:text>※  </xsl:text>  全残第二顺序受益人为被保险人本人。<xsl:text></xsl:text>
				 </xsl:otherwise>
				 </xsl:choose>
				 <xsl:variable name="num" select="count(Bnf)" /> 
				 <xsl:choose>
				 <xsl:when test="$num = 3">
				 <xsl:text>※  </xsl:text>  身故第二顺序受益人：法定<xsl:text>※※</xsl:text>
				 </xsl:when>
				 <xsl:otherwise>
				 <xsl:choose>
		...
		<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_($Amnt,15,$Falseflag)"/><xsl:text>元</xsl:text>
 		<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_($Prem,11,$Falseflag)"/>元</xsl:for-each>
 		<xsl:text>※※※※ </xsl:text><xsl:text>  保险期间起止时间：</xsl:text>自<xsl:value-of select="java:com.sinosoft.midplat.common.DateUtilZR.date8to11(/TranData/Body/Risk/CValiDate)"/><xsl:text>零时起至</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.DateUtilZR.date8to11(/TranData/Body/Risk/InsuEndDate)"/><xsl:text>二十四时止</xsl:text>
		...
		<xsl:text>※※※    </xsl:text><xsl:text>  保险合同号: </xsl:text><xsl:value-of select="/TranData/Body/ContNo"/>
	   	...
	   	<xsl:text>※    </xsl:text><xsl:text>  保险条款等相关资料，经审核确认保险合同内容正确无误。本人已阅读过产品条款，确认已了解并认</xsl:text>
	   	<xsl:text>※    </xsl:text><xsl:text>  可保险合同的全部内容，知晓本人的权利和义务。</xsl:text>
	   	<xsl:text>※  </xsl:text><xsl:text></xsl:text>
