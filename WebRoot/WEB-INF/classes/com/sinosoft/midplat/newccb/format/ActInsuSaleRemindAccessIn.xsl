...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="TX_BODY/ENTITY/APP_ENTITY">
		<Body>
		    <!-- #查询日期 -->
			<BagName><xsl:value-of select="AgIns_BtchBag_Nm" /></BagName>
			<QryStrDate></QryStrDate>
			<!-- 本地节点 -->
			<secNodeId><xsl:value-of select="//TX_HEADER/LocalID"/></secNodeId>
			<!-- 银行节点 -->
			<rmtSecNodeId><xsl:value-of select="//TX_HEADER/remoteID"/></rmtSecNodeId>	
		</Body>
	</xsl:template>
