		...
		<!-- 报文体 -->
			<TX_BODY>
	      			...
	      			<ENTITY>
			        	<APP_ENTITY>
							 <!-- 险种编号 -->
							<Cvr_ID><xsl:value-of select="/TranData/Body/Risk[RiskCode=MainRiskCode]/RiskCode" /></Cvr_ID>	
							<!--保单号 -->
							<InsPolcy_No><xsl:value-of select="TranData/Body/ContNo"/></InsPolcy_No>
			        		<!-- #循环记录条数 PDF文件个数 -->
			   				<Rvl_Rcrd_Num_1>0</Rvl_Rcrd_Num_1>
			        		<!-- 返回文件数量 -->
			        		<Rvl_Rcrd_Num>3</Rvl_Rcrd_Num>
							<Detail_List>
								<!-- 提示信息描述 -->
								...
								<!-- 保险重空编号 -->
								<Ins_IBVoch_ID><xsl:value-of select="TranData/Body/ContPrtNo"/></Ins_IBVoch_ID>
								<!-- #循环记录条数 -->
