	...
	<xsl:template match="/">
		...
		<!-- 报文体 -->
			<TX_BODY>
	      			...
	      			<ENTITY>
			        	<APP_ENTITY>
			        	<Ins_Co_ID>010072</Ins_Co_ID><!-- 保险公司编号-->
                        <Ins_Co_Nm>中韩人寿</Ins_Co_Nm><!-- 保险公司名称 -->
                      <!--   <Rvl_Rcrd_Num><xsl:value-of select="/TranData/Body/RiskCount"/></Rvl_Rcrd_Num>#循环记录条数  -->
                        <xsl:choose>
                        <xsl:when test="/TranData/Body/RiskCount = '0'"></xsl:when>
                        <xsl:when test="/TranData/Body/RiskCount ! = '0'">
			        		 <Life_List>
				        		<xsl:for-each select="/TranData/Body/Risk">
				        			<Life_Detail>
					        			...
					        			<Cvr_ID><xsl:value-of select="RiskCode" /></Cvr_ID>
					        		      <!-- 险种名称 -->
					        			 <Cvr_Nm><xsl:value-of select="RiskNm" /></Cvr_Nm>		
					        			<!-- 保费金额 -->
			        		...
			        		<!-- 总保费金额 -->
			        		<Tot_InsPrem_Amt><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fenToYuan(/TranData/Body/TotalPrem)"/></Tot_InsPrem_Amt> 
			        		<!-- 首期缴费金额 -->
			        		<Init_PyF_Amt><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fenToYuan(/TranData/Body/Risk/Prem)"/></Init_PyF_Amt>
			        		<!-- 年化保费金额 -->
			        		<Anulz_InsPrem_Amt><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fenToYuan(/TranData/Body/AnnualPrem)"/></Anulz_InsPrem_Amt>
			                <!-- 保费缴费期数 -->
