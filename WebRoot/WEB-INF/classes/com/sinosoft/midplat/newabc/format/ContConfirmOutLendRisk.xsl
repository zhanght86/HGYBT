...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:java="http://xml.apache.org/xslt/java" exclude-result-prefixes="java">
	...
	<xsl:template name="Base" match="Body">
				...
				<xsl:choose>
				 ...
				 <Prnt>
					...
					<xsl:text>　　　 </xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(RiskName, 32)"/>
																								<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(LendRiskDay, 3,$Falseflag)"/><xsl:text>天</xsl:text>
																			<xsl:text>      </xsl:text>
																			<xsl:choose>
																			...
																			<xsl:apply-templates select="PayIntv"/>
																			 ...
																			 <xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_($Prem,12,$Falseflag)"/>元</Prnt>
					</xsl:for-each>
					<Prnt />
					<Prnt />
					<Prnt><xsl:text>　　　 </xsl:text><xsl:text>保险期间起止时间：</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.DateUtilZR.date8to11(Risk/CValiDate)"/><xsl:text>零时起至</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.DateUtilZR.date8to11(Risk/InsuEndDate)"/><xsl:text>二十四时止</xsl:text></Prnt>  
		...
		<TbdPrnts>
					       ...
					       <Prnt/>
					       <Prnt/>
					       <Prnt><xsl:text>　     </xsl:text><xsl:text>保险合同号: </xsl:text><xsl:value-of select="/TranData/Body/ContNo"/></Prnt>
					       <Prnt><xsl:text>　     </xsl:text><xsl:text>投保人: </xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(/TranData/Body/Appnt/Name, 19)"/><xsl:text>银保经理姓名：      </xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(/TranData/Body/AgentName, 18)"/><xsl:text>银保经理代码：</xsl:text><xsl:value-of select="/TranData/Body/AgentCode"/></Prnt>
						   <Prnt><xsl:text>　     </xsl:text><xsl:text>     本投保人已收到贵公司的保险合同（保险合同号: </xsl:text><xsl:value-of select="/TranData/Body/ContNo"/><xsl:text>），本保险合同包括保险单，</xsl:text></Prnt>
						   <Prnt><xsl:text>　     </xsl:text><xsl:text>保险条款等相关资料，经审核确认保险合同内容正确无误。本人已阅读过产品条款，确认已了解并认可</xsl:text></Prnt>
						   <Prnt><xsl:text>　     </xsl:text><xsl:text>保险合同的全部内容，知晓本人的权利和义务。</xsl:text></Prnt>
						   ...
						   <Prnt><xsl:text>　     </xsl:text><xsl:text>------------------------------------------------------------------------------------------------</xsl:text></Prnt>
						   <Prnt><xsl:text>　     </xsl:text><xsl:text>    保险合同号为 </xsl:text><xsl:value-of select="/TranData/Body/ContNo"/><xsl:text>的保险合同已送达客户，由客户亲笔签字确认，现将保险合同送达</xsl:text></Prnt>
