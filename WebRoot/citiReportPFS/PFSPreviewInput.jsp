...
<%
	...
	filePath += "/temp/Midplat_PFS_Report_Preview.xls";
	ExcelUtils eu = new ExcelUtils("PFS", filePath,"PFSPreview");
	String startDay = request.getParameter("StartDay");
	String endDay = request.getParameter("EndDay");
	
	System.out.println("startDay:"+startDay+",endDay:"+endDay);
	System.out.println("PFS:客户正在提取每日实时数据，文件路径为：" + filePath);
	...
	String Content = "";  
	String FlagStr = "";
	PolicyFundSummary policyFundS = new PolicyFundSummary();
	
	try{
	if (!policyFundS.submitData(tVData, "download")) {
		FlagStr = "Fail";
		Content = policyFundS.mErrors.getFirstError().toString();

	} else {
		FlagStr = "Succ";
		Content = (String) policyFundS.getResult().get(0);
		System.out.println("PFS报表生成成功");
		file = (File) policyFundS.getResult().get(1);
