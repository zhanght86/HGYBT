...
function  verifyInput(){
	...
	var vVCHNO = fm.mAgInSr_BtWthldBag_Num.value; 
	if (vVCHNO.trim() == ""){    
		alert("代理保险剩余批量代扣包个数不能为空");
		return false;  
	}
	
	
	var vCONACCNO = fm.mAlrdy_Pcsg_ADdBhBgNum.value; 
	if (vCONACCNO.trim() == ""){    
		alert("已处理代扣批量包个数不能为空");
		return false;  
	}
	var vSALER = fm.mAgInSr_BtSbPyBag_Num.value; 
	if (vSALER.trim() == ""){    
		alert("代理保险剩余批量代付包个数不能为空");
		return false;  
	}

	var vSALER2 = fm.mAlrdy_Pcsg_SPyBhBgNum.value; 
	if (vSALER2.trim() == ""){    
		alert("已处理代付批量包个数不能为空");
		return false;  
	}
