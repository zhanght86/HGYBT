<%@ page contentType="text/html; charset=gb2312" language="java" import="java.sql.*,java.util.*,com.sinosoft.utility.*" errorPage="" %>
	...
	if(!sbmt){//Deal with this Event only once;
		sbmt=true;
                findInDoc("Comcode").value=cId;
								...
								//top.opener.fm.all('ComCode').value = trim(cId);
                //top.opener.fm.all('UpComCode').value = trim(cId);
				 
                top.opener.afterQuery(trim(cId));
                top.close();
	}
}
</script>
