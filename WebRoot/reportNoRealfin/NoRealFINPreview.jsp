...
<frameset name="fraMain" rows="0,0,0,0,*" frameborder="no" border="1" framespacing="0" cols="*">
	...
	<frameset name="fraSet" cols="0%,*,0%" frameborder="no" border="1" framespacing="0" rows="*">
		...
		<frame id="fraInterface" name="fraInterface" scrolling="auto" src="./NoRealFINPreviewInput.jsp?TranCom=<%=sTranCom%>&AreaNo=<%=sArea%>&CityNo=<%=sCity%>&ContStatue=<%=sContStatue%>&StartDay=<%=sStartDay%>&EndDay=<%=sEndDay%>&StartHour=<%=sStartHour%>&EndHour=<%=sEndHour%>&ManageCodeNo=<%=sManageCodeNo%>">
		<frame>
