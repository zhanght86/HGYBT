...
/**
 * @ClassName: SpdbGetPLDSRespBatch
 * @Description: 批量代扣回盘文件
 * @author sinosoft
 * @date 2017-4-21 下午5:32:23
 */
public class SpdbGetPLDSRespBatch extends SpdbGetFileFromBankSvc
{

	public SpdbGetPLDSRespBatch()
	{
		super(SpdbConf.newInstance(), "3014");
	}
	...
	protected void deal(File cLocalFile)
	{
		...
		cLogger.info("Into SpdbGetPLDSRespBatch.deal()...个性化业务处理模块...");
		// 发送请求报文至核心
		...
		try
		{
			...
			Document cOutXmlDoc = new CallWebsvcAtomSvc(AblifeCodeDef.SID_Bank_spdbpldsResp).call(cInXmlDoc);
	...
	{
		cLogger.info("Into SpdbGetPLDSRespBatch.sendRequest()...");
		ElementLis TranData = new ElementLis("TranData");
		Element Head = getHead();
		TranData.addContent(Head);
		Document pXmlDoc = new Document(TranData);
		cLogger.info("Out SpdbGetPLDSRespBatch.sendRequest()..." + cTranDate);
		return pXmlDoc;
	}

	public static void main(String[] args) throws Exception
	{
		SpdbGetPLDSRespBatch tBlc = new SpdbGetPLDSRespBatch();
		Thread.currentThread().setName(String.valueOf(NoFactory.nextTranLogNo()));
		Logger mLogger = Logger.getLogger(SpdbGetPLDSRespBatch.class);

		mLogger.info("浦发批量代收下载批处理程序开始...");
