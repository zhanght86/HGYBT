...
//			.append(" FuncFlag=").append("1014")
			.append(" tranno ='").append(mOldTranNo).append('\'')
			.append(" and MakeDate='").append(cTranLogDB.getMakeDate()).append('\'')
...
//			.append(" and ZoneNo='").append(cTranLogDB.getZoneNo())
			...
			mFuncflag = String.valueOf(tTranLogSchema.getFuncFlag());
			...
			ContSchema tContSchema	= new ContSchema();
			String tSqlStr=null;
			if(mFuncflag.equals("1014")){
				//当天、同一网点，成功出过单
...
//				.toString();
				...
				throw new MidplatException("只能对新单确认交易进行冲正！");
			}
		} catch (Exception ex) {
