	...
	<xsl:template match="/">
			...
			<Transaction_Body>
				...
				<Detail_List>
					...
					<xsl:otherwise>
					<BkDetail1><xsl:text>           </xsl:text>（本栏空白）</BkDetail1>
					<BkDetail1/>
					</xsl:otherwise>
					</xsl:choose>
					<xsl:choose>
					...
					<BkDetail1 />
					<BkDetail1 />
					<BkDetail1><xsl:text>　　　　　　　　　　 </xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_('　', 36,$Falseflag)"/><xsl:value-of select="java:com.sinosoft.midplat.common.DateUtilZR.date8to11($MainRisk/SignDate)"/></BkDetail1>
					<BkDetail1 /> 
				    </Detail>
					 </Detail_List>
					 <Detail_List>
