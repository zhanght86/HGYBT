...
public class BatchSendDisk extends XmlSimpFormat {
	...
	public Document noStd2Std(Document pNoStdXml) throws Exception {
		...
		// 克隆非标准输入报文头
		cTransaction_Header = (Element) pNoStdXml.getRootElement()
				.getChild("TX_HEADER").clone();

		// JdomUtil.print(cTransaction_Header);

		// 服务接受时间[简单日期格式化当前日期]
		mSYS_RECV_TIME = new SimpleDateFormat("yyyyMMddHHmmssSSS")
				.format(new Date());
		// 克隆非标准输入报文头
		oldTxHeader = (Element) pNoStdXml.getRootElement()
				.getChild("TX_HEADER").clone();
		// 克隆非标准输入报文公共域
		oldComEntity = (Element) pNoStdXml.getRootElement().getChild("TX_BODY")
				.getChild("ENTITY").getChild("COM_ENTITY").clone();
		// 非标准输入报文头服务码
		sysTxCode = oldTxHeader.getChildText("SYS_TX_CODE");

		// 临时保存保险公司方交易流水号
		// 非标准输入报文公共域服务方流水号
		tranNo = pNoStdXml.getRootElement().getChild("TX_BODY")
				.getChild("ENTITY").getChild("COM_ENTITY")
				.getChildText("SvPt_Jrnl_No");
		// 临时保存银行发起交易日期作为保险公司账务日期
		// 非标准输入报文头发起方交易时间前8位
		tranDate = NewCcbFormatUtil.getTimeAndDate(pNoStdXml.getRootElement()
				.getChild("TX_HEADER").getChildText("SYS_REQ_TIME"), 0, 8);
		// 非标准输入报文应用域代理保险批量包名称
		tBagName = pNoStdXml.getRootElement().getChild("TX_BODY")
...
//				.setText(tBagName.substring(10, 18));
	...
	public Document std2NoStd(Document pStdXml) throws Exception {
		...
		// 服务状态
		Element mSYS_TX_STATUS = mNoStdXml.getRootElement()
				.getChild("TX_HEADER").getChild("SYS_TX_STATUS");
		// 服务响应描述
		Element mSYS_RESP_DESC = mNoStdXml.getRootElement()
				.getChild("TX_HEADER").getChild("SYS_RESP_DESC");

		/* End-组织返回报文头 */
		Element tAPP_ENTITY = mNoStdXml.getRootElement().getChild("TX_BODY")
				.getChild("ENTITY").getChild("APP_ENTITY");
		...
		String tTranCode = cThisBusiConf.getChild("funcFlag")
				.getAttributeValue("outcode");
		...
		mNoStdXml.getRootElement().getChild("TX_BODY").getChild("COMMON")
				.getChild("FILE_LIST_PACK").getChild("FILE_INFO")
				.getChild("FILE_NAME").setText(fileName);
		mNoStdXml.getRootElement().getChild("TX_BODY").getChild("COMMON")
				.getChild("FILE_LIST_PACK").getChild("FILE_INFO")
				.getChild("FILE_PATH").setText(ccblocalPath);
		// 每次请求取数文件只有一个，所以该FILE_NUM设置为1
		mNoStdXml.getRootElement().getChild("TX_BODY").getChild("COMMON")
				.getChild("FILE_LIST_PACK").getChild("FILE_NUM").setText("1");
		mNoStdXml.getRootElement().getChild("TX_BODY").getChild("COMMON")
				.getChild("FILE_LIST_PACK").getChild("FILE_MODE").setText("0");
		...
		if (tDetail.size() != 0) {
			...
			boolean isTrue = new PutContFile().isSuccessed(tDetail, filePath,
					tBagName, mNoStdXml, cNoStdXml);
			if (isTrue = true) {
				// 当前批明细总笔数
				Element tCur_Btch_Dtl_TDnum = tAPP_ENTITY.getChild(
						"Cur_Btch_Dtl_TDnum").setText(
						Integer.toString(tDetail.size()));
				// 批量包名称
				Element AgIns_BtchBag_Nm = tAPP_ENTITY.getChild(
						"AgIns_BtchBag_Nm").setText(tBagName);
			} else {
