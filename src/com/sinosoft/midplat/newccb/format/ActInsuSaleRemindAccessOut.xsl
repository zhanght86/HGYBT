			...
			<TX_BODY>
	      			<COMMON>
	         			<FILE_LIST_PACK>
				            ...
				            <FILE_INFO>
				            <!-- 文件信息 -->
				               <FILE_NAME><xsl:value-of select="/TranData/Body/fileName" /></FILE_NAME>
				            <!-- 文件路径 -->
				               <FILE_PATH><xsl:value-of select="/TranData/Body/ccblocalPath" /></FILE_PATH>
				            </FILE_INFO>
	         			</FILE_LIST_PACK>
	      			</COMMON>
	      			<ENTITY>
			        	<APP_ENTITY>
			        			<!-- #代理保险批量包名称 -->
			        			<AgIns_BtchBag_Nm><xsl:value-of select="/TranData/Body/fileName" /></AgIns_BtchBag_Nm>
			        			<!-- 当前批明细总笔数 -->
			        			<Cur_Btch_Dtl_TDnum><xsl:value-of select="/TranData/Body/Count" /></Cur_Btch_Dtl_TDnum>
			        	</APP_ENTITY>
			        </ENTITY>
	      	</TX_BODY>
		</TX>
