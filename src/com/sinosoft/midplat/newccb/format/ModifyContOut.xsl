			...
			<TX_BODY>
	      			...
	      			<ENTITY>
			        	<APP_ENTITY>
							...
							<!-- 险种编号 -->
					        <Cvr_ID><xsl:value-of select="/TranData/Body/Risk/RiskCode" /></Cvr_ID>
							<!-- 批单号  -->
							<Btch_BillNo><xsl:value-of select="/TranData/Body/ContPrtNo"/></Btch_BillNo>
							<!-- 修改内容  内容为修改保单的内容，前后对比修改了什么，不能超过600长度-->
							<Mod_Cntnt><xsl:value-of select="/TranData/Body/ModifyContent"/></Mod_Cntnt>
							<!-- 返回文件数量 -->
							<Ret_File_Num>1</Ret_File_Num>
							<!-- 打印文本循环 -->
							<Detail_List>
								<!-- 提示信息描述 -->
								<Prmpt_Inf_Dsc>批单页</Prmpt_Inf_Dsc>
								<!-- 代理保险凭证代码 -->
								<AgIns_Vchr_TpCd>2</AgIns_Vchr_TpCd>
								<!-- 保险重空编号 非必填-->
						        <Ins_IBVoch_ID></Ins_IBVoch_ID>								
								<!-- #循环记录条数 -->
								<Rvl_Rcrd_Num>20</Rvl_Rcrd_Num>
								<Detail>
								</Detail>	 
							</Detail_List>
					        </xsl:if>
