...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="APP_ENTITY">
		<Body>
			<FileName><xsl:value-of select="//AgIns_BtchBag_Nm" /></FileName>
			<BatchNo><xsl:value-of select="//BtNo_SN" /></BatchNo>
			<Err_ID><xsl:value-of select="//Err_ID" /></Err_ID>
			<ERR_DSC><xsl:value-of select="//ERR_DSC" /></ERR_DSC>
		</Body>
	</xsl:template>
