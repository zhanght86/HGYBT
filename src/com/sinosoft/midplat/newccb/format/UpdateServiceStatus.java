...
//		JdomUtil.print(pStdXml);
		...
		if (mRetData.getChildText(Flag).equals("1")) { // 交易成功[设置转换后的非标准报文头]
		    ...
		    String BAK1=null;//建行非实时投保交易保费缴费方式代码|保费缴费期数|保费缴费周期代码
			for (int i = 0; i <LCContList.size(); i++) {
			    ...
			    Insu_DetailEle.getChild("InsPrem_PyF_MtdCd").setText(strs[0]);
			    Insu_DetailEle.getChild("InsPrem_PyF_Prd_Num").setText(strs[1]);
			    Insu_DetailEle.getChild("InsPrem_PyF_Cyc_Cd").setText(strs[2]);
			}
