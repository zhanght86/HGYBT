	...
	<xsl:template match="TX_BODY/ENTITY/APP_ENTITY">
		<Body>
			...
			<ProposalPrtNo><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.no13To15(Ins_Bl_Prt_No)" /></ProposalPrtNo>
  			...
  			<ContPrtNo><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.no13To15(BkVchNo)" /></ContPrtNo>   
			<!-- 投保日期 -->
...
<!-- 被保人 -->
			...
			<!-- 被保人姓名 -->
				...
				<!-- 被保人证件生效日期
				<xsl:choose>
				<xsl:when test="Rcgn_Crdt_EfDt=20991231" ><RcgnCrdtEfDt>99990101</RcgnCrdtEfDt></xsl:when>
				<xsl:otherwise><RcgnCrdtEfDt><xsl:value-of select="Rcgn_Crdt_EfDt"/></RcgnCrdtEfDt></xsl:otherwise>
				</xsl:choose>-->
				<!-- 被保人证件失效日期-->
				<xsl:choose>
				...
				<!-- 被保人职业代码-->
				<JobCode><xsl:value-of select="Rcgn_Ocp_Cd" /></JobCode>
				<!-- 被保人年收入 -->
			...
			<!-- 		<xsl:for-each select="Benf_Detail">    建行自己发的测试报文没有Benf_List/Benf_Detail只有 Benf_Detail -->	
						...
						<!-- 受益人证件生效日期-->
						<xsl:choose>
						<xsl:when test="Benf_Crdt_EfDt=20991231" ><BenfCrdtEfDt>99990101</BenfCrdtEfDt></xsl:when>
						<xsl:otherwise><BenfCrdtEfDt><xsl:value-of select="Benf_Crdt_EfDt"/></BenfCrdtEfDt></xsl:otherwise>
						</xsl:choose>
						<!-- 受益人证件失效日期-->
						<xsl:choose>
			...
			<Risk>
				...
				<!-- 缴费年期和缴费年龄 -->
				 <xsl:if test="InsPrem_PyF_MtdCd = 0"><!-- 趸交1000Y给核心 -->				    
					<PayEndYear>1000</PayEndYear>
					<PayEndYearFlag>Y</PayEndYearFlag>
				</xsl:if>
				<xsl:if test="InsPrem_PyF_MtdCd != 0">	<!-- 建行没有交费年期类型一说 -->
				...
				<!-- 年金领取周期代码 
                <Anuty_Drw_Cyc_Cd><xsl:value-of select="Anuty_Drw_Cyc_Cd" /></Anuty_Drw_Cyc_Cd>-->				
				<!-- 保险年期类别代码 -->
				<InsuYearFlag>
	...
	<xsl:template name="apntInsPayType" >
		...
		<xsl:if test="$ApntInsPayType = 0">N</xsl:if><!-- 不垫交 -->
		<xsl:if test="$ApntInsPayType = 1">Y</xsl:if><!-- 自动垫交 -->
	</xsl:template>
	<!-- 性别 -->
	...
	<xsl:template name="tran_relation">
		...
		<xsl:if test="$RelaToInsured = 0133043">00</xsl:if><!--本人 -->
		<xsl:if test="$RelaToInsured = 0133010">02</xsl:if><!--配偶 -->
		<xsl:if test="$RelaToInsured = 0133015">01</xsl:if><!--父亲  -->
		<xsl:if test="$RelaToInsured = 0133016">01</xsl:if><!--母亲 -->
		<xsl:if test="$RelaToInsured = 0133011">03</xsl:if><!--儿子 -->
		<xsl:if test="$RelaToInsured = 0133012">03</xsl:if><!--女儿-->
		<xsl:if test="$RelaToInsured = 0133017">04</xsl:if><!--祖父-->
		<xsl:if test="$RelaToInsured = 0133018">04</xsl:if><!--祖母-->
		<xsl:if test="$RelaToInsured = 0133013">04</xsl:if><!--孙子 -->
		<xsl:if test="$RelaToInsured = 0133014">04</xsl:if><!--孙女 -->
		<xsl:if test="$RelaToInsured = 0133020">06</xsl:if><!--哥哥 -->
		<xsl:if test="$RelaToInsured = 0133019">06</xsl:if><!--姐姐-->
		<xsl:if test="$RelaToInsured = 0133021">06</xsl:if><!--其它亲属 -->
		<xsl:if test="$RelaToInsured = 0133002">06</xsl:if><!--同事 -->
		<xsl:if test="$RelaToInsured = 0133001">06</xsl:if><!--朋友--><!-- 核心没朋友啊，只能为其他喽 Update 20130226 -->
	</xsl:template>
