			...
			<TX_BODY>
	      			...
	      			<ENTITY>
			        	<APP_ENTITY>
							...
							<!-- 批单号  -->
							<Btch_BillNo><xsl:value-of select="/TranData/Body/CardNo"/></Btch_BillNo>					        
							<!-- 返回文件数量  -->
							<Ret_File_Num></Ret_File_Num>
							<xsl:for-each select="/TranData/Body/AttachmentData">
						        <Detail_List>
						        	<!-- 提示信息描述  -->
						        	<Prmpt_Inf_Dsc>满期给付信息：</Prmpt_Inf_Dsc>
						        	<!-- 代理保险凭证类型代码 -->
						        	<!-- 	1	保单
											2	批单
											3	现金价值单
