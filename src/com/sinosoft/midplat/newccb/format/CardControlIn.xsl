...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:java="http://xml.apache.org/xslt/java" exclude-result-prefixes="java">
	...
	<xsl:template match="TX">
		...
		<TranData>
			...
			<!-- 保险重空起始案号 -->
			<StartNo>
				<xsl:value-of select="TX_BODY/ENTITY/APP_ENTITY/Ins_IBVoch_Beg_ID"/>
			</StartNo>
			<!-- 保险重空结束案号 -->
			<EndNo>
				<xsl:value-of select="TX_BODY/ENTITY/APP_ENTITY/Ins_IBVoch_End_ID"/>
			</EndNo>
		</Body>
	</TranData>
</xsl:template>
...
<xsl:template name="tran_type" >
	...
	<xsl:if test="$Type = '010072000001'">2104141</xsl:if>
	<!-- 投保单号 -->
	<xsl:if test="$Type = '010072000002'">0101141</xsl:if>
	<!-- 保单印刷号 -->
</xsl:template>
