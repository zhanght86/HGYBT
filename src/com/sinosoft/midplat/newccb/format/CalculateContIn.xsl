...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:java="http://xml.apache.org/xslt/java" exclude-result-prefixes="java">
	...
	<xsl:template match="TX_BODY/ENTITY/APP_ENTITY">
		<Body>
			...
			<!-- 主险保额 -->
			<Amnt>
				<xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.yuanToFen(MainIns_Cvr)" />
			</Amnt>
			<!-- 保费金额 -->
			<Prem>
	...
	<xsl:template name="tran_Contpayintv">
		...
		<xsl:choose>
			<xsl:when test="$payintv = 9999">D</xsl:when>
			<!-- 不定期交 -->
			...
			<!-- 季交 -->
			<xsl:when test="$payintv = 0202">Y</xsl:when>
			<!-- 半年交 -->
			<xsl:when test="$payintv = 0203">Y</xsl:when>
			<!-- 年交 -->
			<xsl:when test="$payintv = 0401">A</xsl:when>
			<!-- 交至某确定年 -->
