					...
					<!-- 	<xsl:for-each select="/TranData/Body/Detail_List">
							...
							<Detail_List>
								...
								<xsl:text>         </xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(RiskName, 36)"/>
								 ...
								 <Ret_Inf/>
								 <Ret_Inf/>
							<!-- 	 <Ret_Inf><xsl:text>         </xsl:text>保险期间起止时间：自<xsl:value-of select="java:com.sinosoft.midplat.common.DateUtilZR.date8to11(/TranData/Body/Risk/CValiDate)"/><xsl:text>零时起至</xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.DateUtilZR.date8to11(/TranData/Body/Risk/InsuEndDate)"/><xsl:text>二十四时止</xsl:text></Ret_Inf>   -->
