...
public class NewContConfirm extends ServiceImpl {
	...
	public Document service(Document pInXmlDoc) {
		...
		try {
			...
			//校验非实时出单
			String getFuncFlag = "select FuncFlag from TranLog where ProposalPrtNo='"+mProposalPrtNo+"' and RCode = 0";
			cLogger.info("查询非实时投保交易码:"+getFuncFlag);
			String mFuncFlag = new ExeSQL().getOneValue(getFuncFlag);
			cLogger.info("非实时投保交易码:"+mFuncFlag);
			ContSchema tContSchema =null;
			String tSqlStr =null;
			//非实时数据不做次校验
			if(!"1060".equals(mFuncFlag)){
				// 当天、同一网点，成功录过单
				tSqlStr = new StringBuilder("select * from Cont where Type=")
			...
			if(!"1060".equals(mFuncFlag)){
				...
				cLogger.info("更新保单SQL语句："+tSqlStr);
