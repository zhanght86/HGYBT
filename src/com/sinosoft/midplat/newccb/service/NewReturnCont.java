...
public class NewReturnCont extends ServiceImpl {
	public NewReturnCont(Element pThisBusiConf) {
		super(pThisBusiConf);
	}
	...
	public Document service(Document pInXmlDoc) {
		...
		cLogger.info("Into NewReturnCont.service()...");
		cInXmlDoc = pInXmlDoc;
...
//			JdomUtil.print(cInXmlDoc);
			...
			cLogger.info("Into NewReturnCont.service()...-->authority(cInXmlDoc)网点与权限 添加代理");	
...
//			new RuleParser().check(cInXmlDoc);
			...
			cOutXmlDoc = new CallWebsvcAtomSvc(AblifeCodeDef.SID_CCBReturnCont).call(cInXmlDoc);
			...
			if (tUseTime > tTimeOut*1000) {
				cLogger.error("处理超时！UseTime=" + tUseTime/1000.0 + "s；TimeOut=" + tTimeOut + "s；投保书：" + "");
				rollback();	//回滚系统数据
		...
		cLogger.info("Out NewReturnCont.service()!");
		return cOutXmlDoc;
	}
	
	private void rollback() {
		cLogger.debug("Into NewReturnCont.rollback()...");
		...
		cLogger.debug("Out NewReturnCont.rollback()!");
	}
