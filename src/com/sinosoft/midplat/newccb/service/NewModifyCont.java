...
public class NewModifyCont extends ServiceImpl {
	public NewModifyCont(Element pThisBusiConf) {
		super(pThisBusiConf);
	}
	...
	public Document service(Document pInXmlDoc) {
		...
		cLogger.info("Into NewModifyCont.service()...");
		cInXmlDoc = pInXmlDoc;
...
//			cInXmlDoc = authority(cInXmlDoc); 	
			...
			cOutXmlDoc = new CallWebsvcAtomSvc(AblifeCodeDef.SID_Bank_QueryContModify).call(cInXmlDoc);
			System.out.println("-----------------------------------------------");
...
//			cTranLogDB.setContNo(tContDB.getContNo());
		...
		cLogger.info("Out NewModifyCont.service()!");
		return cOutXmlDoc;
	}
	
	private ContDB getContDB() {
		cLogger.debug("Into NewModifyCont.getContDB()...");
		
		
		
		Element mInBodyEle = cOutXmlDoc.getRootElement().getChild(Body);
...
//JdomUtil.print(cOutXmlDoc);
		...
		if(!mInRiskEle.getChildText(MainRiskCode).equals("")&&mInRiskEle.getChildText(MainRiskCode)!=null){
			mContDB.setBak1(mInRiskEle.getChildText(MainRiskCode));
		}
		mContDB.setOperator(CodeDef.SYS);
		
		cLogger.debug("Out NewModifyCont.getContDB()!");
		return mContDB;
	}
	
	private void rollback() {
		cLogger.debug("Into NewModifyCont.rollback()...");
		...
		cLogger.debug("Out NewModifyCont.rollback()!");
	}
	/**
	 * @param mInXmlDoc
	 * @return cInXmlDoc
