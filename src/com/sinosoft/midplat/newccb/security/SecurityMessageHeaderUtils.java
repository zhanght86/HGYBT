...
public class SecurityMessageHeaderUtils {
	...
	public static final String log003 = "服务{0}远程安全节点{1}发送请求时调用安全出错,错误码为{2}";
	public static final String log004 = "服务{0}远程安全节点{1}响应时调用安全出错,错误码为{2}";
	public static final String log005 = "发送安全报文头信息=\n{0}";
...
//							new Object[] { t.getLocalizedMessage() });
		...
		try {
			...
			//安全组件退出
//			System.out.println("退出安全节点号为："+secNodeId);
//			SecAPI.nodeFinal(secNodeId);
//			System.out.println("退出安全节点完成");
		} catch (Throwable t) {
...
//			//安全组件初始化
			try{
				System.out.println("初始化安全节点号为："+secNodeId);
				SecAPI.nodeInit(secNodeId);
				System.out.println("初始化安全节点完成");
			}catch(Exception e){
				e.printStackTrace();
				System.out.println("初始化安全节点失败。。。");
			}
...
//				System.out.println("校验MAC。。。。。。。。。。。。");
			...
			//进行安全上下文、MAC、报文解密
			if (isMac) {
//				System.out.println("MAC 值："+new String((byte[]) container.get("SEC_MAC")));
...
//			e.printStackTrace();
//		}
//		System.out.println("本地安全节点号："+mRmtSecNodeId +"      对端安全节点号："+mSecNodeId +"      " +
//				"解密内容："+new String(cipherData));
		byte[] secDecryptBytes =  SecurityManagerUtils.pinDecrypt(mRmtSecNodeId, mSecNodeId, cipherData);
		
//		System.out.println("解密后值为："+new String(secDecryptBytes));
