...
public class BatRequResult extends ServiceImpl {
	public BatRequResult(Element pThisBusiConf) {
		super(pThisBusiConf);
	}
	
	public Document service(Document pInXmlDoc) {
		long mStartMillis = System.currentTimeMillis();
		cLogger.info("Into BatRequResult.service()...");
		cInXmlDoc = pInXmlDoc;
		...
		try {
			...
			cLogger.info("Into BatRequResult.service()...-->authorityCheck.submitData(mHeadEle)交易权限");	
			...
			tFlag.setText(cOutXmlDoc.getRootElement().getChild(Body).getChildText(Flag));
			Element tDesc = new Element(Desc); 
			tDesc.setText(cOutXmlDoc.getRootElement().getChild(Body).getChildText(Desc));
		...
		cLogger.info("Out BatRequResult.service()!");
		return cOutXmlDoc;
	}
}
