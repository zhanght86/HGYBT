...
public class HandContBankRst extends ServiceImpl {
	public HandContBankRst(Element pThisBusiConf) {
		super(pThisBusiConf);
	} 
	
	public Document service(Document pInXmlDoc) {
		long mStartMillis = System.currentTimeMillis();
		cLogger.info("Into HandContBankRst.service()...");
		cInXmlDoc = pInXmlDoc; 
...
//			} 
			...
			//20141225lilu回盘交易 是去取银行端sftp的回盘文件，前置机写入tranlog，置状态为成功，表示成功接受银行回盘文件，不需要经过核心
//			cOutXmlDoc = new CallWebsvcAtomSvc(AblifeCodeDef.SID_Bank_NewContBlc).call(cInXmlDoc);
		...
		cLogger.info("Out HandContBankRst.service()!");
		return cOutXmlDoc;
	}
	...
	protected ContBlcDtlSet saveDetails(Document pXmlDoc) throws Exception {
		cLogger.debug("Into HandContBankRst.saveDetails()...");
...
//			tContBlcDtlSchema.setOperator(CodeDef.SYS);
		...
		cLogger.debug("Out HandContBankRst.saveDetails()!");
