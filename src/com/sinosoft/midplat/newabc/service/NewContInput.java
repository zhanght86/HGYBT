...
//			JdomUtil.print(cInXmlDoc);
			...
			.append(" and bak3='").append(mApplyNo).append('\'')
			...
			if ("1".equals(new ExeSQL().getOneValue(tResendSql))) {
				...
				try {
					...
					.append(" and makedate=").append(DateUtil.get8Date(tCurCalendar))
					.toString();
					String tDltContSql= new StringBuilder("delete from cont where proposalprtno='")
					.append(mProposalPrtNo).append('\'')
					.append(" and makedate=").append(DateUtil.get8Date(tCurCalendar))
					.toString();
					new ExeSQL().execUpdateSQL(tDltTranlogSql);
					new ExeSQL().execUpdateSQL(tDltContSql);
				} catch (Exception e) {
					e.printStackTrace();
				}
			...
			if("211902".equals(tRiskCode.getTextTrim())){//安赢借贷险才判断
						...
						String applyno=cInXmlDoc.getRootElement().getChild("Body").getChildText("ApplyNo");
						String sqlapplyno="select count(1) from cont where bak8='"+applyno+"' and trancom ='05'";
						String count=new ExeSQL().getOneValue(sqlapplyno);
						if (!count.equals("0")) {
