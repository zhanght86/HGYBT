...
public class SurRetrDataDocBankDeal extends ServiceImpl
{
	public SurRetrDataDocBankDeal(Element pThisBusiConf)
	{
		super(pThisBusiConf);
	}
	...
	protected ContBlcDtlSet saveDetails(Document pXmlDoc) throws Exception
	{
		...
		for (Element tDetailEle : mDetailList)
		{
			...
			//交易类型
			tContBlcDtlSchema.setBak3(tDetailEle.getChildText("BusiType"));
		 ...
		 */
		cLogger.info("退保犹退回盘总数(DtlSet)为：" + mContBlcDtlSet.size());
		...
		{
			cLogger.error("保存退保犹退回盘明细失败！" + mPubSubmit.mErrors.getFirstError());
			throw new MidplatException("保存退保犹退回盘明细失败！");
		}
