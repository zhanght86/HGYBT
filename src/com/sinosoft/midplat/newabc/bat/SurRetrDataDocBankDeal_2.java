 ...
 *
 */
public class SurRetrDataDocBankDeal_2 extends Balance{
	
	private String stranNo = null;
	
	public SurRetrDataDocBankDeal_2() {
	...
	protected Element parse(InputStream pBatIs) throws Exception {
		...
		if("3103".equals(mHeadMsgs.substring(0, 4))){//如果回盘正确，首行为记录；错误的时候为错误信息
			...
			for (String tLineMsg; null != (tLineMsg=mBufReader.readLine());) {
				...
				if("240000".equals(tSubMsgs[13])){//银行受理状态成功取错误码
				...
				if("01".equals(tSubMsgs[8])){//犹撤
					tBusiType.setText("07");
				}
				if("02".equals(tSubMsgs[8])){//满期
					tBusiType.setText("09");
				}
				if("03".equals(tSubMsgs[8])){//退保
	...
	public static void main(String[] args) throws Exception {
		...
		SurRetrDataDocBankDeal_2 mBatch = new SurRetrDataDocBankDeal_2();
		//用于补对账，设置补对账日期
		if (0 != args.length) {
