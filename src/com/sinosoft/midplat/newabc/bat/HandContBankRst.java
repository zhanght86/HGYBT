 ...
 *
 */
public class HandContBankRst extends Balance{
	public HandContBankRst() {
		super(NewAbcConf.newInstance(), "2012");
	}
	protected String getFileName()throws Exception {
		Element mBankEle = cThisConfRoot.getChild("bank");
		File_download f = new File_download(cThisBusiConf,"SGDJGHP",DateUtil.getDateStr(cTranDate,"yyyyMMdd"),mBankEle.getAttributeValue("insu"));
		String fileName="SRESULT.BANK"+mBankEle.getAttributeValue("insu")+"."+DateUtil.getDateStr(cTranDate, "yyyyMMdd");
		try {
			f.bank_dz_file();
		} catch (Exception ex) {
			ex.printStackTrace();
	 ...
	 public void run()
	   {
	        ...
	        this.cLogger.info("Into HandContBankRst.run()...");
	    ...
	    try
	    {
	       ...
	       this.cLogger.info("Out HandContBankRst.run()!");
	   }
	
	protected Element parse(InputStream pBatIs) throws Exception {
		cLogger.info("Into HandContBankRst.parse()...");
		...
		/*	文件第一行：（汇总信息）
			...
			文件其他内容：（明细记录）
			公司代码|银行方流水号|保单号|银行省市代码|网点号|投保单号|投保人证件类型|投保人证件号码|投保人名称|投保人与被保人关系|
			被保人证件类型|被保人证件号码|被保人名称|投保份数|总保费|投保日期|保单到期日|承保日期|缴费账户|银行人员资格证编号|
			个性化费率|保单印刷号|附加险个数错误码|错误信息|备注1|备注2|
		*/
...
//			}
			 ...
			 */
			String nodeNo=null;
			if(tSubMsgs[3]!=null&&tSubMsgs[4]!=null){
				nodeNo=tSubMsgs[3].trim()+tSubMsgs[4].trim();
			}
			
			Element tTranDateEle = new Element(TranDate);
			tTranDateEle.setText(tSubMsgs[15]);
...
//			tDetailEle.addContent(tProposalPrtNoEle);
		...
		cLogger.info("Out HandContBankRst.parse()!");
		return mBodyEle;
	}
	
	protected Element getHead() {
		cLogger.info("Into HandContBankRst.getHead()...");
		...
		cLogger.info("Out HandContBankRst.getHead()!");
		return mHead;
	}
	
	
	public static void main(String[] args) throws Exception {
		Logger mLogger = Logger.getLogger("com.sinosoft.midplat.newabc.bat.HandContBankRst.main");
		mLogger.info("程序开始...");
		
		HandContBankRst mBatch = new HandContBankRst();
		//用于补对账，设置补对账日期
		if (0 != args.length) {
