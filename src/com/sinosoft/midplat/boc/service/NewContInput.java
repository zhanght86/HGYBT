...
public class NewContInput extends ServiceImpl {
	...
	public Document service(Document pInXmlDoc) {
		...
		try {
			...
			//校验系统中是否有相同保单正在处理，尚未返回,默认超时设置为5分钟(300s)；如果未配置锁定时间，则使用该值。
			int tLockTime = 300;
			try {
				//使用默认值
...
//			if("211901".equals(tRiskCode.getTextTrim())||"211902".equals(tRiskCode.getTextTrim())){
			//农行的借贷险用这种更细化的校验方式
...
//			}
			...
			if("0".equals(cOutXmlDoc.getRootElement().getChild("Head").getChildText("Flag"))){
				//保存保单信息
