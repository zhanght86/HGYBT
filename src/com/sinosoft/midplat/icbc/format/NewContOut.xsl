		...
		<SubVoucher>
			...
			<Text>
				...
				<PageNum>1</PageNum>
					...
					<TextRowContent><xsl:text></xsl:text><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_(RiskName, 34)"/>
																					<xsl:choose>
																							...
																							<xsl:when test="InsuYearFlag = 'Y'">
																								<xsl:if test = "InsuYear = '1000'"><xsl:value-of select="java:com.sinosoft.midplat.common.NumberUtil.fillStrWith_('保终身', 2,$Falseflag)"/></xsl:if>
																								<xsl:if test = "InsuYear != '1000'">
					...
					<xsl:if test="Risk[RiskCode=MainRiskCode]/RiskCode != '221301'"> <!-- 悦未来不显示该栏 -->
					<TextRowContent><xsl:text></xsl:text>------------------------------------------------------------------------------------------------</TextRowContent>
					<TextRowContent ><xsl:choose><xsl:when test="Risk[RiskCode=MainRiskCode]/RiskCode != '221201' and Risk[RiskCode=MainRiskCode]/RiskCode !='221301' and Risk[RiskCode=MainRiskCode]/RiskCode !='121504'">
...
<xsl:template name="Cashs" match="CashValues">
        <xsl:if test="/TranData/Body/Risk[RiskCode=MainRiskCode]/RiskCode != '221301' and /TranData/Body/Risk[RiskCode=MainRiskCode]/RiskCode != '121504'">    <!--  悦未来年金险现价可能超100 -->
