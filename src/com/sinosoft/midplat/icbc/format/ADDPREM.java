...
public class ADDPREM extends XmlSimpFormat {
	private Element cTXLifeRequest = null;
	public ADDPREM(Element pThisBusiConf) {
		super(pThisBusiConf);
	} 
	...
	public Document noStd2Std(Document pNoStdXml) throws Exception {
		...
		Document mStdXml = 
			ADDPREMInXsl.newInstance().getCache().transform(pNoStdXml);
		
		cLogger.info("Out ADDPREM.noStd2Std()!");
		return mStdXml;
	}
	
	public Document std2NoStd(Document pStdXml) throws Exception {
		cLogger.info("Into ADDPREM.std2NoStd()...");
		
		Document mNoStdXml = 
			ADDPREMOutXsl.newInstance().getCache().transform(pStdXml);
		...
		Element mPhoneNumber = new Element("PhoneNumber");//保险公司热线电话
		mPhoneNumber.setText(cTXLifeRequest.getChildText("PhoneNumber"));//处理标记
		...
		mTXLifeResponse.addContent(mPhoneNumber);

		
		cLogger.info("Out ADDPREM.std2NoStd()!");
		return mNoStdXml;
	}
...
//		Document mOutXmlDoc = new ADDPREM(null).std2NoStd(mInXmlDoc);
		Document mOutXmlDoc = new ADDPREM(null).noStd2Std(mInXmlDoc);
