...
public class IFLogSchema implements Schema
{
	// @Field
	/** 日志号 */
	private int LogNo;
	...
	{
		if (Area != null && !Area.equals("") && SysConst.CHANGECHARSET == true)
		{
			Area = StrTool.unicodeToGBK(Area);
		}
		return Area;
	}
	/** 区域 */
	...
	/**
	* 使用另外一个 IFLogSchema 对象给 Schema 赋值
	* @param: Schema 对象
	* @return: 无
	**/
	public void setSchema(IFLogSchema aIFLogSchema)
	{
		this.LogNo = aIFLogSchema.getLogNo();
		this.IFtype = aIFLogSchema.getIFtype();
		this.Operator = aIFLogSchema.getOperator();
		this.RCode = aIFLogSchema.getRCode();
		this.Area = aIFLogSchema.getArea();
		this.TranDate = fDate.getDate( aIFLogSchema.getTranDate());
		this.FilePath = aIFLogSchema.getFilePath();
		this.FileName = aIFLogSchema.getFileName();
		this.FileNo = aIFLogSchema.getFileNo();
		this.Bak1 = aIFLogSchema.getBak1();
		this.Bak2 = aIFLogSchema.getBak2();
		this.Bak3 = aIFLogSchema.getBak3();
		this.MakeDate = fDate.getDate( aIFLogSchema.getMakeDate());
		this.MakeTime = aIFLogSchema.getMakeTime();
	}
	...
	**/
	...
	public IFLogSchema getSchema()
	{
		IFLogSchema aIFLogSchema = new IFLogSchema();
		aIFLogSchema.setSchema(this);
		return aIFLogSchema;
	}

	public IFLogDB getDB()
	{
		IFLogDB aDBOper = new IFLogDB();
	...
	**/
	public String encode()
	{
		...
		strReturn =  ChgData.chgData(LogNo) + SysConst.PACKAGESPILTER
					...
					+ StrTool.cTrim( StrTool.unicodeToGBK(Area) ) + SysConst.PACKAGESPILTER
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpIFLog>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			Area = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			...
			FileNo= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,9,SysConst.PACKAGESPILTER))).intValue();
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		IFLogSchema other = (IFLogSchema)otherObject;
		return
