...
import com.sinosoft.lis.db.LKCardDB;

public class LKCardSchema implements Schema
{
	// @Field
	/** 单证号 */
	...
	/**
	* 使用另外一个 LKCardSchema 对象给 Schema 赋值
	* @param: Schema 对象
	* @return: 无
	**/
	public void setSchema(LKCardSchema aLKCardSchema)
	{
		this.CardNo = aLKCardSchema.getCardNo();
		this.Type = aLKCardSchema.getType();
		this.State = aLKCardSchema.getState();
		this.ContNo = aLKCardSchema.getContNo();
		this.ProposalNo = aLKCardSchema.getProposalNo();
		this.AgentCom = aLKCardSchema.getAgentCom();
		this.AgentCode = aLKCardSchema.getAgentCode();
		this.TranCom = aLKCardSchema.getTranCom();
		this.ZoneNo = aLKCardSchema.getZoneNo();
		this.NodeNo = aLKCardSchema.getNodeNo();
		this.ManageCom = aLKCardSchema.getManageCom();
		this.Bak1 = aLKCardSchema.getBak1();
		this.Bak2 = aLKCardSchema.getBak2();
		this.Bak3 = aLKCardSchema.getBak3();
		this.MakeDate = fDate.getDate( aLKCardSchema.getMakeDate());
		this.MakeTime = aLKCardSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLKCardSchema.getModifyDate());
		this.ModifyTime = aLKCardSchema.getModifyTime();
		this.Operator = aLKCardSchema.getOperator();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		catch(SQLException sqle)
		{
			...
			tError.moduleName = "LKCardSchema";
	...
	public LKCardSchema getSchema()
	{
		LKCardSchema aLKCardSchema = new LKCardSchema();
		aLKCardSchema.setSchema(this);
		return aLKCardSchema;
	}

	public LKCardDB getDB()
	{
		LKCardDB aDBOper = new LKCardDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLKCard描述/A>表字段
	* @param: 无
	* @return: 返回打包后字符串
	**/
	...
	{
		String strReturn = "";
		strReturn = StrTool.cTrim( StrTool.unicodeToGBK(CardNo) ) + SysConst.PACKAGESPILTER
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLKCard>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			State= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,3,SysConst.PACKAGESPILTER))).intValue();
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LKCardSchema other = (LKCardSchema)otherObject;
		return
