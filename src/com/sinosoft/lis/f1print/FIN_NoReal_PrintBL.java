...
/**
 * <p>
 * ClassName: FIN_NoReal_PrintBL
 * </p>
 * <p>
 * Description: 银保通每日非实时数据导出表
 * </p>
 * <p>
...
public class FIN_NoReal_PrintBL extends FINCreatExcel {
	...
	private boolean getPrintData() {
		...
		if(sContStatue==null || "".equals(sContStatue)){
			title2 = "***所有状态***";
			sContStatueSql = "  (c.axauwflag in ('03','09','11','X','Y'))";
		}else if(sContStatue!=null && "03".equals(sContStatue)){
			title2 = "***客户取消申请***";
			sContStatueSql = "  (c.axauwflag='03'  and c.uwflag='7')";
		}
		else if(sContStatue!=null && "09".equals(sContStatue)){
			title2 = "***己签发保单***";
			sContStatueSql = "  (c.axauwflag='09' and c.uwflag='9') ";
		}
		else if(sContStatue!=null && "11".equals(sContStatue)){
			title2 = "***公司取消申请***";
			sContStatueSql = "  (c.axauwflag='11' and c.uwflag='11') ";
		}
		else if(sContStatue!=null && "X".equals(sContStatue)){
			title2 = "***延期受理***";
			sContStatueSql = "  (c.axauwflag='X' and c.uwflag='12') ";
		}
		else if(sContStatue!=null && "Y".equals(sContStatue)){
			title2 = "***拒保***";
			sContStatueSql = "  (c.axauwflag='Y' and c.uwflag='11' ) ";
		}
		...
		if (sManageCom != null && !"".equals(sManageCom)) {
			sSBSql.append(" and C.managecom like '" + sManageCom + "%'");
		}
		 sSBSql.append(" AND (TO_CHAR(C.UWDATE,'YYYY-MM-DD')||' '||C.UWTIME) >= '" + this.sStartDay + "' AND  (TO_CHAR(C.UWDATE,'YYYY-MM-DD')||' '||C.UWTIME) <= '" + this.sEndDay + "'");
	...
	public static String getSql (String agentcode,String managecom,String sContStatueSql){
		...
		sSql += " SELECT "
			 ...
			 + "  nvl((select codename from ldcode where codetype='noreal_state' and code = c.axauwflag),'转非实时核保') 保单状态,"
		...
		sSql +=" UNION "
			...
			+ "  sum(nvl((SELECT CASE WHEN sp.riskcode in ('NHYrider(RTU)','HYG3rider(RTU)') THEN sp.prem ELSE 0 END 定期投资 FROM LCPOL sp WHERE (sp.polno=c.contno||'-1' or sp.polno=c.contno||'-2') AND sp.payendyear!=1 AND sp.riskcode in ('NHYrider(RTU)', 'HYG3rider(RTU)')),0)),"
			 + " sum((select nvl(p.firstaddprem,0)+nvl((SELECT CASE WHEN sp.riskcode in ('NHYrider(lpsm)','HYG3rider(lpsm)') THEN sp.prem ELSE 0 END 首期投资 FROM LCPOL sp WHERE (sp.polno=c.contno||'-1' or sp.polno=c.contno||'-2') AND sp.payendyear=1 and sp.riskcode in ('NHYrider(lpsm)','HYG3rider(lpsm)')),0) from dual)) "
			 + "  FROM LCCONT C, LCPOL P "
			 + "  WHERE "
			 + "  C.CONTNO||'-0' = P.polno"
			 + "  AND " + sContStatueSql
	...
	public static String [][] getString1(String sTranCom,  String tManageCom, String sStartDay, String sEndDay, String sContStatueSql, String sStartHour, String sEndHour){
		...
		if (tManageCom != null && !"".equals(tManageCom)) {
			sSBSql.append(" and C.managecom like '" + tManageCom + "%' ");
		}
	    sSBSql.append(" AND (TO_CHAR(C.UWDATE,'YYYY-MM-DD')||' '||C.UWTIME) >= '" + sStartDay + "' AND  (TO_CHAR(C.UWDATE,'YYYY-MM-DD')||' '||C.UWTIME) <= '" + sEndDay + "'");
		StringBuffer sSBSql2 = new StringBuffer();
		sSBSql2.append(sSBSql.toString());
