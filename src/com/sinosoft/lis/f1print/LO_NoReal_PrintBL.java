...
public class LO_NoReal_PrintBL extends CreatExcel {
	...
	private boolean getPrintData() {
		...
		setMergeCells(0, 0, 70, 0);
		setBorderLineStyle(1, 0, BorderLineStyle.THIN);
		setMergeCells(0, 1, 70, 1);
		...
		String[] head = { "保单编号", "保单状态","对账状态", "网点代码", "网点编号", "网点名称", "购买日期",
				...
				"保险费合计",
				"投资账户", "投资账户比例", "投资日期", "转账帐号", "账户所有人", "工行超时提醒", "下载时间" };
		for (int i = 0; i < 71; i++) {
		...
		sSql += "SELECT TRIM(C.CONTNO) 保单编号,"
				...
				+ "   c.bankaccno 转账账号,"
				+ "   c.accname 账户所有人,"
				+ "   CASE WHEN c.overtimeflag = 'Y' THEN '超时' ELSE '' END 工行超时提醒,"
...
//				+ "		and ((c.norealflag='Y' and c.uwflag='6' and c.axauwflag is null and c.appflag in('1','B'))"
				+ "		and ((c.norealflag='Y' and c.uwflag!='a' and (c.axauwflag is null or c.axauwflag not in('03', '09', '11', 'X', 'Y')))"
				+ "		OR  (c.norealflag='Y' and c.uwflag!='6' and c.axauwflag  in ('03', '09', '11', 'X', 'Y') and c.UWDATE = DATE'" + sDay + "'))";
...
//		if (sDay != null && !"".equals(sDay)) {
//			sSql += "  AND (c.UWDATE = DATE'" + sDay + "' or c.axauwflag is null or c.axauwflag not in ('03','09','11','X','Y'))";
//		}
		sSql += " order by 购买日期 ";
...
//		tTransferData.setNameAndValue("EndDay", sEndDay);
		...
		String Content = "";
		String FlagStr = "";
		LO_NoReal_PrintBL tLO_PrintBL = new LO_NoReal_PrintBL();
