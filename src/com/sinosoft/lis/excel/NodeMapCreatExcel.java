...
public class NodeMapCreatExcel {
	...
	public boolean createExcel() throws Exception {
		...
		for (int i = 0; i < length; i++)// 循环需要创建的sheet。
		{
			...
			if(this.getColorFlag().equals("LO")){
				for (int j = 0; j < sData.length; j++) {
					System.out.println(sData[j][1]);
					if (sData[j][1] != null && "协议终止".equals(sData[j][1])) {
						
						System.out.println(Sheet.getWritableCell(1, j+3).getContents());
						Label lable = (Label) Sheet.getWritableCell(1, j+3);
						Label lable2 = (Label) Sheet.getWritableCell(0, j+3);
