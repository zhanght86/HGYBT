...
public class FINCreatExcel {
	...
	private WritableCellFormat getDataFormat2() throws WriteException {
		WritableFont wf = new WritableFont(WritableFont.ARIAL, 10,
				WritableFont.NO_BOLD, false);
		WritableCellFormat dataFormat = new WritableCellFormat(wf);
		dataFormat.setAlignment(Alignment.CENTRE);
	...
	private WritableCellFormat getDataFormat3() throws WriteException {
		WritableFont wf = new WritableFont(WritableFont.ARIAL, 10,
				WritableFont.NO_BOLD, false);
		WritableCellFormat dataFormat = new WritableCellFormat(wf);
		dataFormat.setAlignment(Alignment.CENTRE);
		dataFormat.setWrap(true);
		dataFormat.setBackground(Colour.IVORY);
		dataFormat.setBorder(Border.ALL, BorderLineStyle.THIN);
		return dataFormat;
	}
	...
	public boolean createExcel() throws Exception {
		...
		for (int i = 0; i < length; i++)// 循环需要创建的sheet。
		{
			...
			titleFormat.setWrap(false);// 设置表头内容可以自动换行--何宁20071012
			...
			for (int n = 0; n < strData.length; n++) {
				...
				for (int m = 0; m < strData[n].length; m++) {
					...
					// System.out.println("列：" + m + "的number:" + numberF);
					if (numberF == null || numberF.equals("")
							|| numberF.equals("null")) {
						// System.out.println("列：" + m + "文本输出");
			...
			for (int j = 0; j < 15; j++) {
				...
				System.out.println(Sheet.getWritableCell(j, 3)
						.getContents());
				Label lable = (Label) Sheet.getWritableCell(j, 3);

				WritableCellFormat cFormat = getDataFormat();
				// 设置背景色
				cFormat.setWrap(true);
				lable.setCellFormat(cFormat);

			}
	
			ArrayList arrayList = new ArrayList();
			for (int j = 0; j < sData.length; j++) {
				System.out.println(sData[j][4]);
				if (sData[j][4] != null && !"网点编号".equals(sData[j][4]) &&!"".equals(sData[j][4])) {
			...
			for (int j = 0; j < arrayList.size(); j++) {
				if((j+1)%2==1){
					for(int z = 0; z<15; z++){
						Label lable = (Label) Sheet.getWritableCell(z, (Integer) arrayList.get(j));
						
						 WritableCellFormat cFormat = getDataFormat2();
						   //设置背景色
				...
				if((j+1)%2==0){
					for(int z = 0; z<15; z++){
						Label lable = (Label) Sheet.getWritableCell(z, (Integer) arrayList.get(j));
						 ...
						 WritableCellFormat cFormat = getDataFormat3();
						   //设置背景色
				...
				for (int j = 0; j < sData.length; j++) {
					System.out.println(sData[j][10]);
					if (sData[j][10] != null && ("协议终止".equals(sData[j][10]) || "对账失败".equals(sData[j][10]))) {
						
						System.out.println(Sheet.getWritableCell(1, j+4).getContents());
						Label lable = (Label) Sheet.getWritableCell(1, j+4);
						Label lable2 = (Label) Sheet.getWritableCell(10, j+4);
						 WritableCellFormat cFormat = getDataFormat();
						   //设置背景色
						    cFormat.setBackground(Colour.RED);
						    lable.setCellFormat(cFormat);
						    lable2.setCellFormat(cFormat);
					}
			}
				...
				for (int j = 0; j < sData.length; j++) {
					System.out.println(sData[j][10]);
					if (sData[j][0] != null && "test1".equals(sData[j][0])) {
						
						 Sheet.mergeCells(0, j+4, 10, j+4);
						 Sheet.mergeCells(11, j+4, 14, j+4);
						 Label lable = (Label) Sheet.getWritableCell(0, j+4);
						 lable.setString("网点每日销售明细");
						   
						 WritableFont wf = new WritableFont(WritableFont.ARIAL, 10,
									WritableFont.NO_BOLD, false);
							WritableCellFormat dataFormat2 = new WritableCellFormat(wf);
							dataFormat2.setAlignment(Alignment.LEFT);
							dataFormat2.setWrap(true);
							dataFormat2.setBorder(Border.ALL, BorderLineStyle.THIN);
						   //设置背景色
						   
						    lable.setCellFormat(dataFormat2);
				...
				for (int j = 0; j < sData.length; j++) {
					System.out.println(sData[j][10]);
					if (sData[j][0] != null && "test2".equals(sData[j][0])) {
						
						 
						 Label lable = (Label) Sheet.getWritableCell(0, j+4);
						 lable.setString("交易流水号");
						 Label lable1 = (Label) Sheet.getWritableCell(1, j+4);
						 lable1.setString("保单编号");
						 Label lable2 = (Label) Sheet.getWritableCell(2, j+4);
						 lable2.setString("险种代码");
						 Label lable3 = (Label) Sheet.getWritableCell(3, j+4);
						 lable3.setString("保费金额");
						 Label lable4 = (Label) Sheet.getWritableCell(4, j+4);
						 lable4.setString("网点编号");
						 Label lable5 = (Label) Sheet.getWritableCell(5, j+4);
						 lable5.setString("代理人编号");
						 Label lable6 = (Label) Sheet.getWritableCell(6, j+4);
						 lable6.setString("标售网点");
						 Label lable7 = (Label) Sheet.getWritableCell(7, j+4);
						 lable7.setString("城市");
						 Label lable8 = (Label) Sheet.getWritableCell(8, j+4);
						 lable8.setString("银行");
						 Label lable9 = (Label) Sheet.getWritableCell(9, j+4);
						 lable9.setString("区域");
						 Label lable10 = (Label) Sheet.getWritableCell(10, j+4);
						 lable10.setString("保单状态");
//						 Label lable11 = (Label) Sheet.getWritableCell(11, j+4);
//						 lable11.setString("对账状态");
						 Label lable12 = (Label) Sheet.getWritableCell(11, j+4);
						 lable12.setString("主险保费");
						 Label lable13 = (Label) Sheet.getWritableCell(12, j+4);
						 lable13.setString("附加险1保费");
						 Label lable14 = (Label) Sheet.getWritableCell(13, j+4);
						 lable14.setString("附加险2保费");
						 Label lable15 = (Label) Sheet.getWritableCell(14, j+4);
						 lable15.setString("首期追加保费");
						 ...
						 WritableFont wf = new WritableFont(WritableFont.ARIAL, 10,
						   ...
						   //设置背景色
							...
							lable.setCellFormat(dataFormat2);
							lable1.setCellFormat(dataFormat2);
							lable2.setCellFormat(dataFormat2);
							lable3.setCellFormat(dataFormat2);
							lable4.setCellFormat(dataFormat2);
							lable5.setCellFormat(dataFormat2);
							lable6.setCellFormat(dataFormat2);
							lable7.setCellFormat(dataFormat2);
							lable8.setCellFormat(dataFormat2);
							lable9.setCellFormat(dataFormat2);
							lable10.setCellFormat(dataFormat2);
//							lable11.setCellFormat(dataFormat2);
							lable12.setCellFormat(dataFormat2);
							lable13.setCellFormat(dataFormat2);
							lable14.setCellFormat(dataFormat2);
							lable15.setCellFormat(dataFormat2);
