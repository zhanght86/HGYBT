...
public class InterFaceCreatExcel {
	...
	private String file = null;// 设置Excel输出文件
	...
	public boolean createExcel() throws Exception {
		File mFile = new File(this.getFile());// 得到Excel创建的路径
		if (!mFile.exists()) {
			mFile.mkdirs();
			if (!mFile.exists()) {
//				cLogger.error("目录不存在，且尝试创建失败！" + mFile.getPath());
//				return;
				throw new Exception("目录不存在，且尝试创建失败！" + mFile.getPath());
			}
		}
