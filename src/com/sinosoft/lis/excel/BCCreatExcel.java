...
public class BCCreatExcel {
		...
		for (int i = 0; i < length; i++)// 循环需要创建的sheet。
		{
			...
			for (int n = 0; n < strData.length; n++) {
				...
				for (int j = 0; j <=15; j++) {
					...
					System.out.println(Sheet.getWritableCell(j, 2)
							.getContents());
					Label lable = (Label) Sheet.getWritableCell(j, 2);
			...
			for (int j = 3; j <sData.length+3; j++) {
					if (j%2 == 1){
					for(int c = 0;c<=15;c++){
					Label lable = (Label) Sheet.getWritableCell(c, j);
	...
	// /*通过sheet编码和行或列的编码组成主键**/
	// private String getKey(int sheet,int rowOrcol)
	// {
	// String strSheet=String.valueOf(sheet);
	// String strRC=String.valueOf(rowOrcol);
	// String key=strSheet+"-"+strRC;
	// return key;
	// }
	//
	// /**通过Hashmap的组件来拆解出sheet编码和行或列的编码***/
	// private String[] getSheetRC(String key)
	// {
	// String []sheetRC=key.split("-");
	// return sheetRC;
	// }
