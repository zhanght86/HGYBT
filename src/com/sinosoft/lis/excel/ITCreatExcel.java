...
public class ITCreatExcel {
	...
	private WritableCellFormat getDataFormat() throws WriteException {
		...
		dataFormat.setBorder(Border.ALL, BorderLineStyle.THICK);
		return dataFormat;
	}
	...
	public boolean createExcel() throws Exception {
		...
		for (int i = 0; i < length; i++)// 循环需要创建的sheet。
		{
			...
			for (int n = 0; n < strData.length; n++) {
				...
				for (int j = 1; j <= 5; j++) {
					System.out.println(Sheet.getWritableCell(j, 1)
							.getContents());
					Label lable = (Label) Sheet.getWritableCell(j, 1);
					WritableCellFormat cFormat = getDataFormat();
					cFormat.setWrap(true);
					lable.setCellFormat(cFormat);
			}
			for (int j = 0; j < sData.length; j++) {
				Element mItReport = MidplatConf.newInstance().getConf()
						.getRootElement().getChild("ItReport");
				String sNumber = mItReport.getAttributeValue("number");
				if (sNumber == null || "".equals(sNumber)) {
					sNumber = "1000";
					System.out.println("没有设置IT报表的 限制值！");
				}
				int iNumber = Integer.valueOf(sNumber);
				Label lable = (Label) Sheet.getWritableCell(4, j + 2);
				Label lable2 = (Label) Sheet.getWritableCell(1, j + 2);
				String sLable = "";
				sLable = Sheet.getWritableCell(5, j + 2).getContents();

				if (sLable == null || "".equals(sLable)) {
					sLable = "0";
				}

				int iLable = Integer.valueOf(sLable);
				if (iLable < iNumber) {
					WritableCellFormat cFormat = getDataFormat();
