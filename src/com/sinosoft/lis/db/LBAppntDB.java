...
public class LBAppntDB extends LBAppntSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LBAppntDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LBAppntSchema tSchema = this.getSchema();
		if (!db.insert(tSchema))
		{
			// @@閿欒澶勭悊
			this.mErrors.copyAllErrors(db.mErrors);
			CError tError = new CError();
			tError.moduleName = "LBAppntDB";
	...
	public boolean getInfo()
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			LBAppntSchema aSchema = this.getSchema();
	...
	public LBAppntSet query()
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			{
				i++;
				LBAppntSchema s1 = new LBAppntSchema();
				s1.setSchema(rs,i);
				aLBAppntSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LBAppntSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBAppntSet aLBAppntSet = new LBAppntSet();
	...
	public LBAppntSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBAppntSet aLBAppntSet = new LBAppntSet();
	...
	public LBAppntSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBAppntSet aLBAppntSet = new LBAppntSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LBAppnt " + sqlObj.getUpdPart() + " where " + strWherePart;
