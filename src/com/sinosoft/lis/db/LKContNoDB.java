...
public class LKContNoDB extends LKContNoSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LKContNoDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LKContNoSchema tSchema = this.getSchema();
		if (!db.insert(tSchema))
		{
			// @@閿欒澶勭悊
			this.mErrors.copyAllErrors(db.mErrors);
			CError tError = new CError();
			tError.moduleName = "LKContNoDB";
	...
	public boolean getInfo()
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("LKContNo");
			LKContNoSchema aSchema = this.getSchema();
	...
	public LKContNoSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		LKContNoSet aLKContNoSet = new LKContNoSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			{
				i++;
				LKContNoSchema s1 = new LKContNoSchema();
				s1.setSchema(rs,i);
				aLKContNoSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LKContNoSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LKContNoSet aLKContNoSet = new LKContNoSet();
	...
	public LKContNoSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LKContNoSet aLKContNoSet = new LKContNoSet();
	...
	public LKContNoSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LKContNoSet aLKContNoSet = new LKContNoSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LKContNo " + sqlObj.getUpdPart() + " where " + strWherePart;
