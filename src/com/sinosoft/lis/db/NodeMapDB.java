...
public class NodeMapDB extends NodeMapSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public NodeMapDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "NodeMap" );
		mflag = true;
	}
	...
	{
		NodeMapSchema tSchema = this.getSchema();
		if (!db.insert(tSchema))
		{
			// @@閿欒澶勭悊
			this.mErrors.copyAllErrors(db.mErrors);
			CError tError = new CError();
			tError.moduleName = "NodeMapDB";
	...
	public boolean getInfo()
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("NodeMap");
			NodeMapSchema aSchema = this.getSchema();
	...
	public NodeMapSet query()
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			{
				i++;
				NodeMapSchema s1 = new NodeMapSchema();
				s1.setSchema(rs,i);
				aNodeMapSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public NodeMapSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		NodeMapSet aNodeMapSet = new NodeMapSet();
	...
	public NodeMapSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		NodeMapSet aNodeMapSet = new NodeMapSet();
	...
	public NodeMapSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		NodeMapSet aNodeMapSet = new NodeMapSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update NodeMap " + sqlObj.getUpdPart() + " where " + strWherePart;
