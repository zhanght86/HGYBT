...
public class AxaAgentDB extends AxaAgentSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public AxaAgentDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		AxaAgentSchema tSchema = this.getSchema();
		if (!db.insert(tSchema))
		{
			// @@閿欒澶勭悊
			this.mErrors.copyAllErrors(db.mErrors);
			CError tError = new CError();
			tError.moduleName = "AxaAgentDB";
	...
	public boolean getInfo()
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("AxaAgent");
			AxaAgentSchema aSchema = this.getSchema();
	...
	public AxaAgentSet query()
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			{
				i++;
				AxaAgentSchema s1 = new AxaAgentSchema();
				s1.setSchema(rs,i);
				aAxaAgentSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public AxaAgentSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		AxaAgentSet aAxaAgentSet = new AxaAgentSet();
	...
	public AxaAgentSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		AxaAgentSet aAxaAgentSet = new AxaAgentSet();
	...
	public AxaAgentSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		AxaAgentSet aAxaAgentSet = new AxaAgentSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update AxaAgent " + sqlObj.getUpdPart() + " where " + strWherePart;
