...
public class AgentVTSParser {
	...
	public boolean transform() {
		...
		try {
			...
			Element eLimitNum = 
		  		 MidplatConf.newInstance().getConf().getRootElement().getChild("AgentImport");
			...
			while (hasMoreData()) {
				i++;
				strFileName = m_strPathName + "\\AgentImportTemp" + ".xml";
	...
	private void genXMLPart(Element eleParent, int sheetNo) throws Exception {
		...
		System.out.println("第一列"+m_book.getText(this.SHEET_CAR, nRow, 0));
		eCityNo.setText(m_book.getText(this.SHEET_CAR, nRow, 0));
		eTop.addContent(eCityNo);
		Element eAgentCode = new Element("AgentCode");
		eAgentCode.setText(m_book.getText(this.SHEET_CAR, nRow, 1));
		eTop.addContent(eAgentCode);
		Element eAgentName = new Element("AgentName");
		eAgentName.setText(m_book.getText(this.SHEET_CAR, nRow, 2));
		eTop.addContent(eAgentName);
		...
		for (nRow = nCurRow, nCount = 0; nRow < nMaxRow; nRow++) {
			...
			if (++nCount > AgentVTSParser.ROW_LIMIT) {
				break;
			}
