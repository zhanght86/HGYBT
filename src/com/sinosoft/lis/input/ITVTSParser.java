...
public class ITVTSParser {
	...
	public boolean transform() {
		...
		try {
			...
			Element eLimitNum = 
		  		 MidplatConf.newInstance().getConf().getRootElement().getChild("ITImport");
			...
			while (hasMoreData()) {
				i++;
				strFileName = m_strPathName + "\\ITImportTemp" + ".xml";
	...
	private void genXMLPart(Element eleParent, int sheetNo) throws Exception {
		...
		Element eContNo = new Element("ContNo");
		eContNo.setText(m_book.getText(this.SHEET_CAR, nRow, 0));
		eTop.addContent(eContNo);
		...
		for (nRow = nCurRow, nCount = 0; nRow < nMaxRow; nRow++) {
			...
			if (++nCount > ITVTSParser.ROW_LIMIT) {
				break;
			}
		...
		for (nRow = nCurRow; nRow < nMaxRow; nRow++) {
			...
			for (int nCol = 0; nCol < nMaxCol; nCol++) {
               try {
				...
				throw new MidplatException("上传的文件不合法！");
			}
			}
